...
const actionDetail = readFileSync(new URL('../src/features/actions/ActionDetail.tsx', import.meta.url), 'utf8')
...
test('Customers and Prospects uses one semantic sortable table with bounded mobile overflow', () => {
  const accounts = readFileSync(new URL('../src/features/accounts/Portfolio.tsx', import.meta.url), 'utf8')
  ...
  for (const label of ['Account', 'Strategic partner', 'BU', 'Health', 'Fit', 'Open items', 'TTM bookings', 'Last CRM activity', 'Owner']) assert.match(accounts, new RegExp(label))
  assert.doesNotMatch(accounts, /'Priority'|'Evidence'|portfolio-summary-grid/)
  assert.doesNotMatch(accounts, /attractivenessSummary\(/)
  assert.match(accounts, /aria-sort=\{descending \? 'descending' : 'ascending'\}/)
  assert.match(accounts, /shown\.slice\(safePage \* PAGE_SIZE/)
  assert.match(accounts, /name\(a\)\.localeCompare\(name\(b\)\)/)
...
test('map and action interactions remain touch-accessible and confirmation-safe', () => {
  ...
  assert.match(actionDetail, /Request approval/)
  assert.match(actionDetail, /Approval does not complete this task/)
...
test('Actions workbench separates Suggestions from durable governed work', () => {
  ...
  assert.match(actionDetail, /Evidence and history/)
  assert.match(actionDetail, /action\.evidence_ids/)
  ...
  assert.match(actionDetail, /action\.allowed_transitions\.map/)
  assert.doesNotMatch(actions + actionDetail, /nextStatuses|priority_rank/)
})
...
test('Actions sends the selected canonical work-item ID without leaking other passive selections', () => {
  ...
  assert.match(actions, /actionId: item\.id/)
...
test('list surfaces publish only their current filters and bounded canonical visible IDs to Omni', () => {
  const portfolio = readFileSync(new URL('../src/features/accounts/Portfolio.tsx', import.meta.url), 'utf8')
  // The curated-only filter box was removed; All now includes every account.
  assert.doesNotMatch(portfolio, /account_scope: 'RICH'/)
  assert.match(portfolio, /market: filters\.market/)
  assert.match(portfolio, /paged\.map\(row => row\.id\)/)
  assert.match(portfolio, /shown\.slice\(safePage \* PAGE_SIZE, \(safePage \+ 1\) \* PAGE_SIZE\)/)
  assert.match(portfolio, /const PAGE_SIZE = 50/)
  assert.match(portfolio, /onOmniContext\(\{ active_filters: Object\.keys\(activeFilters\)\.length \? activeFilters : undefined, visible_record_ids: ids \? ids\.split\('\|'\) : \[\] \}\)/)
  ...
  assert.match(actions, /action_status: status/)
  assert.match(actions, /\(tab === 'suggestions' \? scopedSuggestions : visible\)\.map\(item => item\.id\)\.slice\(0, 50\)/)
  assert.match(actions, /onOmniContext\(\{ active_filters: \{ query, action_status: status, priority \}, visible_record_ids:/)
  ...
  assert.match(today, /filters\.view === 'MARKET_HUBS' \? \[\.\.\.visibleCurrent/)
})
...
test('Seller Command Center reuses governed Signal Briefs with market, Radar, and watch truth', () => {
  assert.match(today, /greetingFor\(new Date\(\)\)/)
  assert.match(today, /No high importance priorities right now/)
  assert.match(today, /highImportanceCount === 1 \? 'priority' : 'priorities'/)
  ...
  assert.match(today, /<Tabs label="Today sections"/)
  assert.match(today, /value: 'PRIORITIES'.*value: 'MARKET_HUBS'/s)
  assert.match(ui, /role="tablist"/)
  assert.match(ui, /role="tab"/)
  assert.match(ui, /ArrowLeft.*ArrowRight.*Home.*End/)
  assert.match(today, /today-source-segment/)
  assert.match(today, /sourceCounts\[kind\]/)
  assert.match(today, /<HighCardinalitySelector.*<SelectInput label="Business unit".*<SelectInput label="Worklist order".*<SearchInput label="Search"/s)
  assert.match(today, /allPriority\.slice\(0, 3\)\.map/)
  assert.match(today, /Counts show current and upcoming public signals/)
  assert.match(today, /disabled=\{count === 0\}/)
  assert.match(today, /No current or upcoming public signals are available across Market Hubs/)
  assert.match(today, /market: next\.market/)
  assert.match(today, /No watched accounts are available/)
  assert.match(today, /No watched programs are available/)
  assert.match(signalBrief, /Analysis pending/)
  assert.match(signalBrief, /Analysis incomplete: no public evidence attached yet/)
  assert.match(signalBrief, /brief\.missing_fields\.join/)
  assert.match(today, /canonicalRank = allPriority\.findIndex/)
  assert.match(today, /Sample data as of/)
  assert.match(uiStyles, /\.ui-tabs.*border-bottom:3px solid transparent/)
  const todayStyles = readFileSync(new URL('../src/features/today/today.css', import.meta.url), 'utf8')
  assert.match(todayStyles, /today-priority-toolbar.*max-width:320px/s)
  assert.match(todayStyles, /today-priority-toolbar.*min-height:36px.*font-size:14px/s)
  assert.match(todayStyles, /pointer:coarse.*min-height:44px/s)
