...
const { scoreValue, commercialDecisionSummary, scorePolarity } = await import(`data:text/javascript;base64,${Buffer.from(output).toString('base64')}`)
...
test('each score family retains its agreed primary display', () => {
  ...
  assert.equal(display('customer_health').value, '72.50')
...
test('color meaning separates risks, opportunity quality and evidence confidence', () => {
  for (const family of ['Attractiveness', 'Opportunity priority', 'Customer health', 'PWIN']) assert.equal(scorePolarity(family), 'positive')
  for (const family of ['Internal commercial risk', 'Overall customer risk', 'Public risk severity']) assert.equal(scorePolarity(family), 'risk')
  for (const family of ['Signal confidence', 'Data coverage', 'unknown family']) assert.equal(scorePolarity(family), 'neutral')
})
