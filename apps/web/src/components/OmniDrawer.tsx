...
const starters = [
  ...
  'Compare these researched Customers.',
...
export function OmniDrawer({ accountId, accountName, context }: { accountId?: string; accountName?: string; context: OmniContext }) {
  ...
  const activeAccount = useMemo(() => accountId && clearedAccountId !== accountId ? { id: accountId, name: accountName ?? 'Selected Customer' } : !accountId && clearedAccountId !== 'SESSION' ? sessionAccount : undefined, [accountId, accountName, clearedAccountId, sessionAccount])
  ...
  return <>
    ...
    <Drawer open={open} onClose={close} titleId="omni-title" className="omni-drawer">
        ...
        <div className="conversation" ref={conversation} aria-live="polite" aria-label="Omni conversation">
          {!messages.length && !loading && <div className="omni-empty"><strong>Start a seller conversation</strong><p>Ask about the curated Customer and Prospect universe, a Customer’s public evidence, a score gap, an action, or nearby map context.</p></div>}
