...
const sellerInterpretation = (value: string) => value.startsWith('IMPLEMENTATION_INTERPRETATION_PENDING_JAMIE_CALIBRATION:')
  ? 'Provisional interpretation pending BTX business calibration; the governed score and its inputs remain unchanged.'
  : value
...
export function ScoreSummary({ model, compact = false }: { model: ScoreSummaryModel; compact?: boolean }) {
  ...
  return <article className={`score-summary ${compact ? 'score-summary-compact' : ''}`} aria-label={`${model.family} score summary`}>
    ...
    {!compact && <><p className="score-summary-decision"><strong>Decision supported:</strong> {model.decision}</p><p>{available ? sellerInterpretation(model.interpretation) : 'A decision score is not available from the current governed inputs.'}</p><dl><div><dt>Subject</dt><dd>{model.subject}</dd></div>{model.asOf && <div><dt>As of</dt><dd>{model.asOf}</dd></div>}</dl></>}
    ...
    {!compact && <Disclosure title="Formula, inputs and supporting evidence">
      ...
      <p className="muted">The configured formula and raw inputs remain governed; this summary does not recalculate the score.{model.version ? ` Score model: ${words(model.version)}.` : ''}</p>
