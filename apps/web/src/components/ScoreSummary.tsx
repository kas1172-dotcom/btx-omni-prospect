...
const displayValue = (value: ScoreSummaryModel['value']) => value == null || value === '' ? 'Unavailable' : typeof value === 'number' ? value.toLocaleString('en-US', { maximumFractionDigits: 2 }) : value
...
export function ScoreSummary({ model, compact = false }: { model: ScoreSummaryModel; compact?: boolean }) {
  ...
  return <article className={`score-summary ${compact ? 'score-summary-compact' : ''}`} aria-label={`${model.family} score summary`}>
    ...
    {!compact && <><p className="score-summary-decision"><strong>Decision supported:</strong> {model.decision}</p><p>{available ? model.interpretation : 'A decision score is not available from the current governed inputs.'}</p><dl><div><dt>Subject</dt><dd>{model.subject}</dd></div>{model.version && <div><dt>Version</dt><dd>{model.version}</dd></div>}{model.asOf && <div><dt>As of</dt><dd>{model.asOf}</dd></div>}</dl></>}
    ...
    {!compact && <Disclosure title="Formula, inputs and supporting evidence">
      ...
      <p className="muted">The configured formula and raw inputs remain governed; this summary does not recalculate the score.</p>
