...
export default function App() {
  ...
  useEffect(() => { void Promise.all([api.accounts(), api.today(), api.intelligence(), api.map()]).then(([accountData, today, intelligence, map]) => { setAccounts(accountData.accounts); setAlerts(today.commercial_alerts); setSignals(intelligence.signals); setRecords(map.records); setLayers(map.layers) }).catch(error => setError(error instanceof Error ? error.message : 'Canonical API unavailable.')).finally(() => setLoading(false)) }, [])
  ...
  return <main className="app-shell"><header className="topbar"><button className="wordmark" onClick={() => setSurface('today')}>BTX <span>OMNI</span></button><span className="mode">SAMPLE · GOVERNED</span><nav aria-label="Primary navigation">{nav.map(([id, label]) => <button key={id} className={surface === id ? 'active' : ''} onClick={() => setSurface(id)}>{label}</button>)}</nav></header>{error && <div className="api-notice">{error}</div>}{content()}<OmniDrawer accountId={detail?.account.id} surface={surface} /></main>
}
