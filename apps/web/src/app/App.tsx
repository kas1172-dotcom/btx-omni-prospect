...
const todayFiltersFromLocation = (location: WorkspaceLocation): TodayFilters => ({ view: location.filters?.view === 'MARKET_HUBS' ? 'MARKET_HUBS' : 'PRIORITIES', market: String(location.filters?.market ?? ''), kind: ['PUBLIC_SIGNAL', 'COMMERCIAL_REVIEW'].includes(String(location.filters?.kind)) ? String(location.filters?.kind) as TodayFilters['kind'] : 'ALL', accountId: String(location.filters?.account ?? ''), businessUnit: String(location.filters?.business_unit ?? ''), query: String(location.filters?.query ?? ''), sort: ['RECENT', 'OLDEST', 'CUSTOMER_ASC', 'CUSTOMER_DESC'].includes(location.sort ?? '') ? location.sort as TodayFilters['sort'] : 'RANKED', page: Number(location.filters?.page ?? 1) || 1, validationPage: Number(location.filters?.validation_page ?? 1) || 1 })
