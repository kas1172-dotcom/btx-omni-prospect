...
export function Map({ records, publicLocations, btxFacilities, layers, signals, onAccount, onMapAccountSelect, onMapFacilitySelect }: { records: MapRecord[]; publicLocations: PublicLocation[]; btxFacilities: BtxMapFacility[]; layers: string[]; signals: MapIntelligence[]; onAccount: (id: string) => void; onMapAccountSelect: (id?: string) => void; onMapFacilitySelect: (facilityId?: string, accountId?: string) => void }) {
  ...
  return <div className="surface map-workspace">
    <div className="page-title map-title"><span className="eyebrow">National commercial geography</span><h1>Tactical Map</h1><p>Verified public locations, BTX facilities, and geographically supported intelligence. Proximity never changes Account Attractiveness.</p></div>
    <section className="map-stage" aria-label="Tactical map workspace">
      ...
      {controlsOpen && <section className="map-controls-panel" id="map-controls" aria-label="Map controls">
        ...
        <div className="control-group"><span>Coverage</span><div className="chips"><button className={coverage === 'RICH' ? 'selected' : ''} onClick={() => applyFilters('RICH', layer, segment)}>Curated scenarios</button><button className={coverage === 'ALL' ? 'selected' : ''} onClick={() => applyFilters('ALL', layer, segment)}>All researched companies</button></div></div>
        ...
        <div className="control-group"><span>Account segment</span><div className="chips" aria-label="Account segment filters">{(Object.keys(segmentLabel) as SegmentFilter[]).map(value => <button className={segment === value ? 'selected' : ''} key={value} onClick={() => applyFilters(coverage, layer, value)}>{segmentLabel[value]}</button>)}</div></div>
        ...
        <div className="map-controls-footer"><small>{visible.length} accounts · {facilities.length} researched facilities · {btxFacilities.length} BTX facilities · {intelligenceMarkers} verified intelligence markers</small>{controlsChanged && <button onClick={clearControls}>Clear filters</button>}</div>
      </section>}
      <div className="map-stage-status"><strong>{visible.length} mapped accounts</strong><span>{facilities.length} researched facilities · {btxFacilities.length} BTX facilities · {showSignals ? intelligenceMarkers : 0} verified intelligence markers</span></div>
      <div className="map-stage-hint">Click a verified marker to inspect its canonical account or facility context.</div>
    </section>
    ...
    </div><aside className="map-quick-view">{selectedRecord ? <Panel title="Account quick view"><State value={selectedRecord.relationship} /><h3>{selectedRecord.name}</h3><p>Markets: {selectedRecord.primary_markets.join(', ')}</p><p>Nearest BTX facility: <strong>{selectedRecord.nearest_btx_facility?.name ?? 'Unavailable'}</strong></p><button className="primary" onClick={() => onAccount(selectedRecord.account_id)}>View Account</button></Panel> : <Panel title="Map preview"><p className="empty">Select an account, facility, or intelligence marker.</p></Panel>}</aside></div> : <Empty>No matching mapped records exist for the selected market, coverage, and account-segment filters.</Empty>}
  </div>
}
