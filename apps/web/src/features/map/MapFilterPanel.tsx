...
export function MapFilterPanel({ filters, onChange, onReset, onClose, changed, markets, naics, filterOptions, fulfillment, planning, planningError, focal }: {
  ...
  return <>
    ...
    <fieldset><legend>Strategic partnership</legend>{planningError && <p role="alert">Saved planning is temporarily unavailable. Existing results remain visible.</p>}{!planning && !planningError && <LoadingStatus>Opening saved planning filters…</LoadingStatus>}<FilterChip disabled={!planning} selected={(filters.strategicPartnership ?? 'ALL') === 'ALL'} onClick={() => onChange({ ...filters, strategicPartnership: 'ALL' })}>All (Customers &amp; Prospects)</FilterChip><FilterChip disabled={!planning} selected={(filters.strategicPartnership ?? 'ALL') === 'EXCLUDE'} onClick={() => onChange({ ...filters, strategicPartnership: filters.strategicPartnership === 'EXCLUDE' ? 'ALL' : 'EXCLUDE' })}>Exclude strategic partnerships</FilterChip><FilterChip disabled={!planning} selected={filters.strategicPartnership === 'ONLY'} onClick={() => onChange({ ...filters, strategicPartnership: filters.strategicPartnership === 'ONLY' ? 'ALL' : 'ONLY' })}>Strategic partnerships only</FilterChip></fieldset>
