...
export function Intelligence({ signals, accounts, onAccount, onEventSelect, onOmniContext }: { signals: Signal[]; accounts: Account[]; onAccount: (id: string) => void; onEventSelect: (id?: string) => void; onOmniContext: (context: Pick<OmniContext, 'active_filters' | 'visible_record_ids'>) => void }) {
  ...
  return <div className="surface intelligence-surface"><div className="page-title intelligence-title"><span className="eyebrow">Curated public evidence</span><h1>Intelligence</h1><p>These briefs are sourced POC research, not live collection. Recommendations stay bounded to the public evidence shown.</p></div><div className="intelligence-summary-grid" aria-label="Intelligence summary"><article><span>Curated signals</span><strong>{signals.length}</strong><small>Canonical public events in this view</small></article><article><span>Canonical account links</span><strong>{linkedAccounts}</strong><small>Only supplied account associations count</small></article><article><span>Source review</span><strong>{needsResearch}</strong><small>Signals still marked needs research</small></article></div><Panel title="Opportunity briefs" action={<span className="panel-kicker">{signals.length} curated events · current canonical order</span>}>{signals.length ? <div className="intelligence-signal-list">{signals.map(signal => <article className={`card intelligence-signal ${selectedEventId === signal.id ? 'selected' : ''}`} key={signal.id}><div className="intelligence-signal-head"><div><span className="eyebrow">{signal.kind.replaceAll('_', ' ')}</span><h3>{name(signal.account_id)}</h3><small>{eventDate(signal.observed_at)} · {signal.source_tier ?? 'Authoritative public source'}</small></div><div className="intelligence-states"><State value="PUBLICLY VERIFIED" /><State value={signal.evidence_state} /></div></div><p className="intelligence-event"><strong>What happened:</strong> {signal.title}</p><div className="intelligence-evidence"><span><strong>Why it may matter</strong>{signal.relevance_explanation}</span><span><strong>Relationship boundary</strong>Not inferred from public evidence</span></div>{sourceValidation(signal.source_validation_state) && <small className="intelligence-validation">Source validation: {sourceValidation(signal.source_validation_state)}</small>}<div className="card-actions intelligence-actions">{usable(signal.source_url) ? <a href={signal.source_url} target="_blank" rel="noreferrer">Open source ↗</a> : <span className="muted">Source unavailable</span>}<button aria-pressed={selectedEventId === signal.id} onClick={() => selectEvent(signal.id)}>{selectedEventId === signal.id ? 'Clear Omni event' : 'Use in Omni'}</button>{signal.account_id && <button onClick={() => onAccount(signal.account_id!)}>Open account</button>}</div><small className="intelligence-next-step">Recommended next step: review public evidence and the account’s explicit POC context.</small></article>)}</div> : <Empty>No curated public evidence is available.</Empty>}</Panel></div>
}
