...
import type { RankedRelationships as Result, RankedRoute, RelationshipMode } from '../../types/relationships'
...
const confirmed = (truth?: string) => ['CONFIRMED', 'VERIFIED', 'VALIDATED', 'PUBLICLY_VERIFIED'].includes(truth ?? '')
const routeVerified = (route: RankedRoute) => route.factor_reasons.length > 0 && route.factor_reasons.every(row => confirmed(row.truth_class)) && route.steps.every(step => !step.unvalidated)
const routeStrength = (route: RankedRoute) => Number(route.factors.coverage) === 1 ? Number(route.utility).toFixed(1) : 'Unknown (range not projected)'
const coverage = (route: RankedRoute) => `Data Coverage ${(Number(route.factors.coverage) * 100).toFixed(0)}%`
...
export function RankedRelationships({ accountId, initialMode, initialPathId, onSelection, onOmniContext }: { accountId: string; initialMode?: string; initialPathId?: string; onSelection?: (pathId: string | undefined, mode: RelationshipMode) => void; onOmniContext: (context: Pick<OmniContext, 'relationship_selection'>) => void }) {
  ...
  const [contactFunction, setContactFunction] = useState('ALL')
  const [contactSeniority, setContactSeniority] = useState('ALL')
  const [contactRecency, setContactRecency] = useState('ALL')
  ...
  const routeContacts = useMemo(() => selected?.steps.filter(step => step.kind === 'external_contact') ?? [], [selected])
  const contactFunctions = [...new Set(routeContacts.map(contact => contact.role_family).filter((value): value is string => Boolean(value)))].sort()
  const contactSeniorities = [...new Set(routeContacts.map(contact => contact.seniority_tier).filter((value): value is string => Boolean(value)))].sort()
  const filteredContacts = routeContacts.filter(contact => {
    const recent = contact.exported_on && result?.scope?.as_of ? new Date(result.scope.as_of).getTime() - new Date(contact.exported_on).getTime() <= 365 * 24 * 60 * 60 * 1000 : false
    return (contactFunction === 'ALL' || contact.role_family === contactFunction)
      && (contactSeniority === 'ALL' || contact.seniority_tier === contactSeniority)
      && (contactRecency === 'ALL' || (contactRecency === 'RECENT') === recent)
  })
  ...
  return <section className="ranked-relationships" aria-label="Ranked canonical relationships" aria-busy={pending}>
    ...
    {selected && <>
      {routeContacts.length > 0 && <section className="network-contact-list" aria-label="Imported contacts on selected route"><strong>Contacts on this route · unvalidated</strong><div className="network-contact-filters"><label>Function<select value={contactFunction} onChange={event => setContactFunction(event.target.value)}><option value="ALL">All functions</option>{contactFunctions.map(value => <option key={value} value={value}>{label(value)}</option>)}</select></label><label>Seniority<select value={contactSeniority} onChange={event => setContactSeniority(event.target.value)}><option value="ALL">All seniority</option>{contactSeniorities.map(value => <option key={value} value={value}>{label(value)}</option>)}</select></label><label>Recency<select value={contactRecency} onChange={event => setContactRecency(event.target.value)}><option value="ALL">Any export date</option><option value="RECENT">Exported within 12 months</option><option value="OLDER">Older or date unavailable</option></select></label></div>{filteredContacts.map(contact => <article key={contact.id}><strong>{contact.label}</strong><span>{label(contact.role_family ?? 'unclassified')} · {label(contact.seniority_tier ?? 'unclassified')}</span><small>{contact.provenance_label}</small><small>Resolution: {label(contact.resolution_method ?? 'unavailable')}</small></article>)}{!filteredContacts.length && <p>No contact on this route matches these filters.</p>}</section>}
      <h3>Selected route · {selected.hop_count} actual edges</h3><p>{routeVerified(selected) ? 'Verified' : 'Hypothetical — validate before use'} · {coverage(selected)}</p>
      <p>{selected.component_context?.map(c => c.label).join(' ↔ ')}</p>
      <ol className="ranked-route-steps">{selected.steps.map((step, index) => <li key={step.id} className={confirmed(selected.assertions?.[index]?.truth_class) ? 'verified-link' : 'unverified-link'}><strong>{step.label}</strong>{index < selected.edge_ids.length && <button disabled={pending || !!error} onClick={() => setSelectedEdge(selected.edge_ids[index])}>{relationshipPredicateLabel(selected.assertions?.[index]?.predicate)} · {relationshipDirectionLabel(selected.inverse_steps[index])} · step {index + 1}</button>}</li>)}</ol>
      ...
      <div className="ranked-network-layout"><div className={`ranked-network ${showGraph ? 'is-open' : ''}`}><header className="ranked-network-head"><div><span className="eyebrow">Relationship field</span><h3>{selected.steps[0]?.label} to {selected.steps.at(-1)?.label}</h3></div><div className="ranked-network-metrics"><span>{selected.hop_count} hops</span><span>{routeStrength(selected)} route strength · {coverage(selected)}</span></div></header>
        ...
        <div ref={stageRef} className="ranked-stage" tabIndex={0} aria-label="Scrollable relationship network; arrow keys scroll"><svg width={stageWidth * zoom} height={stageHeight * zoom} viewBox={`0 0 ${stageWidth} ${stageHeight}`} role="group" aria-label="Canonical relationship network"><defs><marker id={`route-arrow-${accountId}`} viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="currentColor" /></marker><radialGradient id={`origin-halo-${accountId}`}><stop offset="0" stopColor="#36c5f0" stopOpacity=".18" /><stop offset="1" stopColor="#36c5f0" stopOpacity="0" /></radialGradient></defs>
          ...
          {result?.graph.nodes.map(item => { const p = positions.get(item.id); if (!p) return null; const routeIndex = selected.node_ids.indexOf(item.id); const depth = visibleDepths.get(item.id); const networkBadge = item.kind === 'account' && item.unvalidated ? `${item.contact_count ?? 0} contacts · ${item.senior_contact_count ?? 0} senior · ${item.source_people?.join(', ') ?? 'source unavailable'} · unvalidated` : undefined; return <foreignObject key={item.id} data-node-id={item.id} x={p.x - 92} y={p.y - 68} width="184" height="136"><button disabled={pending || !!error} className={`${selected.node_ids.includes(item.id) ? 'active' : ''} node-${item.kind.toLocaleLowerCase().replaceAll('_', '-')}`} aria-pressed={selectedNode === item.id} aria-label={`${item.label}, ${label(item.kind)}${routeIndex >= 0 ? `, ${routeIndex === 0 ? 'route origin' : `hop ${routeIndex}`}` : ''}${networkBadge ? `, ${networkBadge}` : ''}`} onClick={() => focusNode(item.id)}>{routeIndex >= 0 && <b className="ranked-hop-badge">{routeIndex === 0 ? 'O' : routeIndex}</b>}<span className="ranked-node-symbol" aria-hidden="true">{nodeSymbols[item.kind.toLocaleLowerCase()] ?? '•'}</span><span className="ranked-node-label">{item.label}</span><small>{label(item.kind)}{routeIndex < 0 && depth != null ? ` · depth ${depth}` : ''}</small>{networkBadge && <small className="network-contact-badge">{networkBadge}</small>}</button></foreignObject> })}
        </svg></div></div><aside className="ranked-route-inspector" aria-label="Selected relationship explanation"><span className="eyebrow">Omni explanation</span><h3>{selected.steps.at(-1)?.label}</h3><p><strong>Connection</strong>{selected.steps.map(step => step.label).join(' → ')}</p><p><strong>Why this route</strong>{selected.factor_reasons.map(factor => factor.reason).join(' ')}</p><p><strong>Weaker link</strong>{selected.factor_reasons.toSorted((a, b) => a.B - b.B || a.E - b.E)[0]?.reason ?? 'No substantive relationship factor is available.'}</p><p><strong>Supporting evidence</strong>{selected.evidence_ids.length} record{selected.evidence_ids.length === 1 ? '' : 's'} across {selected.hop_count} actual edges.</p>{selected.constraints.length > 0 && <p className="ranked-inspector-constraint"><strong>Execution constraint</strong>{selected.constraints.map(constraint => constraint.reason).join(' ')}</p>}<p><strong>Unverified</strong>{routeVerified(selected) ? 'All displayed edges have verified evidence states; current access still requires confirmation.' : 'Hypothetical route. Unverified links do not establish an introduction, a named contact or route time.'}</p><p><strong>Next action</strong>{selected.next_action}</p>{relatedIntelligence && <section><strong>Relevant intelligence</strong><p>{relatedIntelligence.assessment.headline}</p><p>{relatedIntelligence.assessment.why_it_may_matter}</p><p>{relatedIntelligence.assessment.recommended_action ?? 'Informational only; no action is established.'}</p></section>}<SupportingEvidence count={selected.evidence_ids.length} investigationKey={`relationship:${selected.path_id}:${result?.eligible_graph_revision}`}><p>{coverage(selected)}. The route score is {routeStrength(selected)} under the provisional relationship rubric. It is not a probability, PWIN, qualification or capacity promise.</p><p>{selected.factor_reasons.map(factor => factor.reason).join(' ')}</p><p>{result?.intelligence_boundary}</p></SupportingEvidence></aside></div>
