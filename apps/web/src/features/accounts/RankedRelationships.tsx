...
export function RankedRelationships({ accountId, onOmniContext }: { accountId: string; onOmniContext: (context: Pick<OmniContext, 'relationship_selection'>) => void }) {
  ...
  return <section className="ranked-relationships" aria-label="Ranked canonical relationships" aria-busy={pending}>
    ...
    {result && Object.entries(result.groups).map(([status, group]) => group.routes.length > 0 && <div key={status}><h3>{label(status)}</h3><div className="ranked-route-cards">{group.routes.map(route => <button key={route.path_id} aria-pressed={selected?.path_id === route.path_id} disabled={pending || !!error} onClick={() => selectRoute(route.path_id)}><strong>{route.steps.at(-1)?.label}</strong><span>{route.hop_count} actual edges · {route.factors.bottleneck >= 2 ? 'Scoped experience' : 'Research candidate'}</span><span>{route.next_action}</span></button>)}</div></div>)}
    ...
    {selected && <>
      ...
      <p>{selected.component_context?.map(c => `${c.label} (${c.account_id})`).join(' ↔ ')}</p>
      <ol className="ranked-route-steps">{selected.steps.map((step, index) => <li key={step.id}><strong>{step.label}</strong>{index < selected.edge_ids.length && <button disabled={pending || !!error} onClick={() => setSelectedEdge(selected.edge_ids[index])}>{selected.inverse_steps[index] ? 'Inverse: ' : ''}{label(selected.assertions?.[index]?.predicate ?? 'inspect assertion')} · step {index + 1}</button>}</li>)}</ol>
      ...
      <details><summary>Why this route · factors and evidence</summary><p>Provisional {result?.rubric_version}: utility {Number(selected.utility).toFixed(1)}, not a probability, PWIN or capacity promise.</p>{selected.factor_reasons.map(f => <p key={f.edge_id}>{f.reason} B {f.B}/3 · E {f.E}/3 · F {f.F}/2 · {label(f.truth_class)}</p>)}<p>Evidence: {selected.evidence_ids.join(', ')}</p></details>
      ...
      <div className="ranked-network-layout"><div className={`ranked-network ${showGraph ? 'is-open' : ''}`}><div className="ranked-network-controls"><button onClick={() => setZoom(value => Math.min(2, value + .2))} aria-label="Zoom in">+</button><button onClick={() => setZoom(value => Math.max(.6, value - .2))} aria-label="Zoom out">−</button><button onClick={() => setZoom(1)}>Reset zoom</button><button aria-expanded={expanded} onClick={() => setExpanded(value => !value)}>{expanded ? 'Collapse context' : `Expand context · ${Math.max(0, (result?.graph.nodes.length ?? 0) - selected.node_ids.length)} nodes`}</button><span>Arrows show original assertion direction. Shared nodes are not duplicated.</span></div>
        ...
        <div ref={stageRef} className="ranked-stage" tabIndex={0} aria-label="Scrollable relationship network; arrow keys scroll"><svg width={stageWidth * zoom} height={stageHeight * zoom} viewBox={`0 0 ${stageWidth} ${stageHeight}`} role="group" aria-label="Canonical relationship network"><defs><marker id={`route-arrow-${accountId}`} viewBox="0 0 10 10" refX="10" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="currentColor" /></marker></defs>
          {result?.graph.edges.map(item => {
            ...
            return <g key={item.id}><path d={curve} className={selected.edge_ids.includes(item.id) ? 'active' : ''} markerEnd={`url(#route-arrow-${accountId})`} /><path d={curve} className="edge-hit" role="button" tabIndex={pending || error ? -1 : 0} aria-label={`Inspect ${label(item.predicate)} · ${item.id}`} onClick={() => { if (!pending && !error) setSelectedEdge(item.id) }} onKeyDown={event => { if (!pending && !error && ['Enter', ' '].includes(event.key)) { event.preventDefault(); setSelectedEdge(item.id) } }}><title>{label(item.predicate)} · {item.id}</title></path></g>
          })}
        ...
        </svg></div></div><aside className="ranked-route-inspector" aria-label="Selected relationship explanation"><span className="eyebrow">Selected relationship route</span><h3>{selected.steps.at(-1)?.label}</h3><p><strong>Connection</strong>{selected.steps.map(step => step.label).join(' → ')}</p><p><strong>Why this route</strong>{selected.factor_reasons.map(factor => factor.reason).join(' ')}</p><p><strong>Weaker link</strong>{selected.factor_reasons.toSorted((a, b) => a.B - b.B || a.E - b.E)[0]?.reason ?? 'No substantive relationship factor is available.'}</p><p><strong>Evidence</strong>{selected.evidence_ids.length} governed record{selected.evidence_ids.length === 1 ? '' : 's'} · {selected.hop_count} actual edges · utility {Number(selected.utility).toFixed(1)} under {result?.rubric_version}</p>{selected.constraints.length > 0 && <p className="ranked-inspector-constraint"><strong>Execution constraint</strong>{selected.constraints.map(constraint => constraint.reason).join(' ')}</p>}<p><strong>Next action</strong>{selected.next_action}</p></aside></div>
      ...
      <details><summary>Visible assertions · keyboard and text equivalent</summary>{result?.graph.edges.map(item => <button key={item.id} disabled={pending || !!error} onClick={() => setSelectedEdge(item.id)}>{result.graph.nodes.find(value => value.id === item.source)?.label} → {label(item.predicate)} → {result.graph.nodes.find(value => value.id === item.target)?.label}<small>{item.id} · {item.path_ids?.length ?? 0} returned paths share this assertion</small></button>)}</details>
      {edge && <aside aria-live="polite"><h4>{label(edge.predicate)}</h4><p>Original direction: {edge.source} → {edge.target}</p><p>{label(edge.truth_class)} · observed {edge.observed_on ?? 'date unknown'}</p><label>Supporting record<select value={edge.evidence_ids.includes(evidenceId) ? evidenceId : ''} onChange={e => setEvidenceId(e.target.value)}><option value="">Choose a record to inspect</option>{edge.evidence_ids.map(id => <option key={id} value={id}>{id}</option>)}</select></label>{edge.evidence_ids.includes(evidenceId) && <CommercialEvidence key={`${edge.account_id ?? accountId}:${evidenceId}`} accountId={edge.account_id ?? accountId} recordId={evidenceId} />}<details><summary>Source lineage</summary><p>{edge.lineage_groups.join(', ')}</p></details></aside>}
      <details><summary>All returned routes · ordered text equivalent</summary>{routes.map((route, index) => <button key={route.path_id} data-route-id={route.path_id} aria-pressed={selected?.path_id === route.path_id} disabled={pending || !!error} onClick={() => selectRoute(route.path_id)}>Route {index + 1}: {route.steps.map(s => s.label).join(' → ')} · {route.hop_count} edges<small>{route.assertions?.map(a => `${a.inverse ? 'inverse ' : ''}${label(a.predicate)}`).join(' → ')}</small></button>)}{(result?.additional_route_count ?? 0) > 0 && <p>{result?.additional_route_count} additional evaluated routes are outside this response window.</p>}</details>
    </>}
  </section>
}
