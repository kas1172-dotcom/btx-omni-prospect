...
const defaults = { query: '', type: 'ALL', bu: 'ALL', market: 'ALL', geography: 'ALL', naics: 'ALL', top100: false, band: 'ALL', partnership: 'ALL' }
...
export function Portfolio({ accounts, initialSnapshot, onSnapshot, onSelect, onOmniContext }: { accounts: Account[]; initialSnapshot?: PortfolioSnapshot; onSnapshot?: (snapshot: PortfolioSnapshot) => void; onSelect: (id: string) => void; onOmniContext: (context: Pick<OmniContext, 'active_filters' | 'visible_record_ids'>) => void }) {
  ...
  useEffect(() => { onSnapshot?.({ query: filters.query, scope: 'ALL', industry: filters.market, entity: 'ALL', top100: filters.top100, sortKey: 'name', sortDirection: descending ? 'descending' : 'ascending', filtersOpen: false, page }) }, [filters, descending, page, onSnapshot])
