...
import { ProfileContacts, ProfileFunctionCoverage } from './ProfileContacts'
import { api } from '../../api/client'
import type { Account, Account360, AccountPlanning, AccountRelationships, Action, ActionPriority, FederalAssessment, OmniAssessmentSelection, OmniContext, OmniFederalSelection, SellerRelationshipEvidence, SellerRelationshipPath, Signal } from '../../types/api'
...
import { sameWorkspaceLocation, workspaceHash, type WorkspaceLocation } from '../../app/navigation'
import { relationshipEvidenceLabel } from '../../components/relationshipPresentation'
import { ProfilePage, ProfileTabs } from './ProfileNavigation'
...
function Signals({ items, selectedAssessmentId, onUseInOmni }: { items: Signal[]; selectedAssessmentId?: string; onUseInOmni: (selection?: OmniAssessmentSelection) => void }) {
  return items.length ? <div className="profile-table-wrap"><table className="profile-table"><thead><tr>{['Recent intelligence', 'Source', 'Date', 'Signal Confidence', 'Severity', 'Disposition', 'Site / program', 'Freshness expiry'].map(label => <th key={label}>{label}</th>)}</tr></thead><tbody>{items.map(item => {
    const brief = item.business_briefing; const accountId = brief?.canonical_account_ids[0]
    const selection = brief?.assessment_id && brief.assessment_version && accountId ? { assessment_id: brief.assessment_id, assessment_version: brief.assessment_version, event_id: brief.id, account_id: accountId } : undefined
    return <tr key={item.id}><th scope="row">{brief?.headline ?? item.title}{brief && <details><summary>Assessment and evidence</summary><SignalBriefCard brief={brief} selected={selectedAssessmentId === brief.assessment_id} onUseInOmni={() => selection && onUseInOmni(selectedAssessmentId === selection.assessment_id ? undefined : selection)} /></details>}</th><td>{brief?.source_system ?? item.source_tier ?? 'Unknown'}{(brief?.source_url ?? item.source_url) && <p><a href={brief?.source_url ?? item.source_url} target="_blank" rel="noreferrer">Source</a></p>}{brief?.data_mode ?? item.data_mode}</td><td>{brief?.publication_timestamp?.slice(0, 10) ?? item.observed_at?.slice(0, 10) ?? 'Unknown'}</td><td>{brief?.signal_confidence?.band ?? 'Unknown'}{brief?.signal_confidence && <small>Data Coverage {(Number(brief.signal_confidence.data_coverage.ratio) * 100).toFixed(0)}%</small>}</td><td>{brief?.risk_severity?.band ?? 'Unknown'}</td><td>{brief?.risk_severity?.disposition ?? brief?.seller_promotion_state ?? 'Unknown'}</td><td>{brief?.canonical_facility_id ?? brief?.canonical_program_id ?? item.program_name ?? 'Unknown'}</td><td>{brief?.freshness ?? 'Unknown'}<small>Expiry timestamp not projected</small></td></tr>
  })}</tbody></table></div> : <Empty>No recent public Intelligence assessment is available.</Empty>
}
...
function RelationshipIntelligence({ accountId, federal, location, onLocationChange, onOmniContext }: { accountId: string; federal?: { assessment: FederalAssessment; selection: OmniFederalSelection }; location: WorkspaceLocation; onLocationChange: (next: WorkspaceLocation, mode?: 'push' | 'replace') => void; onOmniContext: (context: Pick<OmniContext, 'relationship_selection'>) => void }) {
  ...
  return <div className="relationship-content">
    ...
    <Disclosure title="Open full Relationship Intelligence workspace" open={workspaceOpen} onOpenChange={open => { setWorkspaceOpen(open); onLocationChange({ ...location, subview: open ? 'relationships' : 'overview', relationship: open ? location.relationship : undefined }, 'push') }}>
    ...
    <details><summary>Reference connections and retained evidence</summary>{best ? <section className="relationship-decision-summary" aria-label="Best supported relationship route"><span className="eyebrow">Best supported route</span><SellerRelationshipCard path={best} /></section> : relationships && <Empty>No eligible recorded route is currently available. This does not establish a real-world absence.</Empty>}
...
const sectionTabs: Record<string, ProfileTab> = {
  'Current intelligence assessment': 'Intelligence', 'Recent intelligence': 'Intelligence',
  'Programs, components & capabilities': 'Overview', 'Opportunities': 'Overview',
  'Federal demand connected to this prospect': 'Overview', 'Opportunities to bring to this customer': 'Overview',
  'Growth & research planning': 'Overview', 'Related BTX activity to review': 'More',
  'Commercial context': 'More', 'Decision panel': 'Commercial', 'Commercial decisions & follow-ups': 'Commercial',
  'Known contacts & CRM': 'Relationships', 'People and relationship paths': 'Relationships', 'Actions': 'Actions',
}
function CustomerSection({ title, summary, children, defaultOpen = true, className = '' }: { title: string; summary?: string; children: ReactNode; defaultOpen?: boolean; className?: string }) { const displaySummary = summary ? presentationLabel(summary) : undefined; return <ProfilePage tab={sectionTabs[title] ?? 'More'}><div className={`customer-section ${className}`}><Disclosure title={<span>{title}{displaySummary && <small>{displaySummary}</small>}</span>} defaultOpen={defaultOpen}>{children}</Disclosure></div></ProfilePage> }
...
function CrmContacts({ detail }: { detail: Account360 }) { const crm = detail.customer_360.crm; return <CustomerSection title="Known contacts & CRM" summary={crm.source_state.data_mode}><SourceState label="CRM" state={crm.source_state} /><ProfileContacts detail={detail} /><MetadataRow label="CRM owner" value={actorDisplayName(crm.owner_id)} /><MetadataRow label="Last CRM activity" value={crm.last_activity_at ?? 'Unknown'} /></CustomerSection> }
function CustomerActions({ accountId, refreshVersion }: { accountId: string; refreshVersion: number }) {
  ...
  useEffect(() => {
    const controller = new AbortController()
    void api.actions(controller.signal).then(result => { if (!controller.signal.aborted) { setItems(result.items.filter(item => item.account_id === accountId)); setLoaded(true) } }).catch(() => { if (!controller.signal.aborted) setNotice('Actions could not be loaded. Previously displayed work may be outdated.') })
  ...
  return <CustomerSection title="Actions" summary={`${items.filter(item => ['OPEN', 'IN_PROGRESS'].includes(item.status)).length} active`}>
    {!loaded && !notice && <LoadingStatus>Loading permitted account Actions…</LoadingStatus>}
    <div className="profile-table-wrap"><table className="profile-table"><thead><tr>{['Action', 'Owner', 'Due', 'Status', 'Linked record'].map(label => <th key={label}>{label}</th>)}</tr></thead><tbody>{items.map(item => <tr key={item.id}><th scope="row">{item.title}</th><td>{actorDisplayName(item.owner_id)}</td><td>{item.due_date ?? 'Not set'}</td><td>{presentationLabel(item.status, 'workflow')} · {presentationLabel(item.approval_status, 'workflow')}</td><td>{item.context_referents.map(([, id]) => id).join(', ') || item.evidence_ids.join(', ') || 'No linked record'}</td></tr>)}</tbody></table></div>
    {loaded && !items.length && <Empty>No Actions for this Customer or Prospect.</Empty>}
    <a href={workspaceHash({ surface: 'actions' })}>Open full Actions screen</a>
...
function AccountPlanningPanel({ accountId, federalOpportunities = [] }: { accountId: string; federalOpportunities?: FederalAssessment[] }) {
  ...
  return <section className="profile-planning-panel" aria-label="Growth & research planning">
    {!planning && !error && <LoadingStatus>Opening saved account planning…</LoadingStatus>}
    <p>Strategic partner: {planning ? designation ? 'Yes' : 'No' : 'Unknown'}</p>{designationRecord && <Notice title="Strategic partnership audit">{designationRecord.reason} Updated {relationshipDate(designationRecord.updated_at)} by {designationRecord.updated_by}.</Notice>}
...
function CustomerDetail({ accounts, detail, initialAssessment, initialFederal, onSelect, onBack, onOmniContext, location, onLocationChange }: { accounts: Account[]; detail: Account360; initialAssessment?: OmniAssessmentSelection; initialFederal?: OmniFederalSelection; onSelect: (id: string) => void; onBack: () => void; onOmniContext: (context: Pick<OmniContext, 'selected_assessment' | 'selected_federal_opportunity' | 'relationship_selection'>) => void; location: WorkspaceLocation; onLocationChange: (next: WorkspaceLocation, mode?: 'push' | 'replace') => void }) {
  ...
  const requestedTab = location.filters?.profileTab
  const profileTab: ProfileTab = location.subview === 'relationships' ? 'Relationships' : location.subview === 'record' ? 'Commercial' : location.subview === 'partnership' ? 'Overview' : profileTabs.find(tab => tab === requestedTab) ?? 'Overview'
  const selectProfileTab = (tab: ProfileTab) => onLocationChange({ ...location, subview: 'overview', filters: { ...location.filters, profileTab: tab } }, 'push')
  return <div className="surface account-detail-surface"><div className="account-workspace-navigation"><Button className="account-workspace-back" variant="ghost" onClick={onBack}>← Customers &amp; Prospects</Button><div className="account-switcher"><SearchInput aria-label="Switch organization" placeholder="Search Customer, Prospect, industry, or location" value={switchQuery} onChange={event => setSwitchQuery(event.target.value)} />{switchResults.length > 0 && <div className="account-switch-results" role="listbox" aria-label="Organization switcher results">{switchResults.map(item => <button type="button" role="option" aria-selected="false" className="account-row account-switch-result" key={item.id} onClick={() => choose(item.id)}><span><strong>{accountName(item)}</strong><small>{item.industries.join(' / ') || 'Industry unavailable'}</small></span><StatusBadge value={classificationLabel(item)} kind="entity" /></button>)}</div>}</div></div><section className="account-workspace" aria-label={`${name} ${organization.title} workspace`}>
    ...
    <div className="profile-header-meta"><span>Business units: {detail.profile.business_unit_ids.join(', ') || 'Unknown'}</span><span>Owner: {detail.profile.owner_id ?? 'Unassigned'}</span><details className="profile-planning-popover"><summary>Strategic partner & shortlist</summary><AccountPlanningPanel key={`planning:${detail.account.id}`} accountId={detail.account.id} federalOpportunities={detail.federal_opportunities} /></details></div>
    <ProfileTabs selected={profileTab} onSelect={selectProfileTab} /><ProfileTabContext.Provider value={profileTab}><div id="profile-tab-content" role="tabpanel" aria-labelledby={`profile-tab-${profileTab}`} tabIndex={0}><ProfilePage tab="Overview">
    <ProfileOverview detail={detail} location={location} />
    </ProfilePage><ProfilePage tab="Overview">{organization.expansion_pursuit && <Notice title="Expansion pursuit">Existing account history is shown beside an unconfirmed new program or component opportunity. Current customer status does not establish participation in this pursuit.</Notice>}</ProfilePage>
    ...
    <ProfilePage tab="Relationships"><ProfileFunctionCoverage detail={detail} /></ProfilePage><div className="account-secondary-grid"><CustomerSection className="account-workspace-relationship" title="People and relationship paths" summary="Recorded and supported connections" defaultOpen><RelationshipIntelligence key={detail.account.id} accountId={detail.account.id} federal={selectedFederal ? (() => { const assessment = detail.federal_opportunities?.find(item => item.assessment_id === selectedFederal.assessment_id && item.assessment_version === selectedFederal.assessment_version); return assessment ? { assessment, selection: selectedFederal } : undefined })() : undefined} location={location} onLocationChange={onLocationChange} onOmniContext={handleRelationshipContext} /></CustomerSection><CustomerSection title="Recent intelligence" summary={`${briefs.length} assessments`}><Signals items={detail.customer_360.intelligence} selectedAssessmentId={selectedAssessment?.assessment_id} onUseInOmni={setSelectedAssessment} /></CustomerSection></div>
...
<CustomerSection title="Opportunities" summary="Potential business, separate from customer health"><p>Review specific expansion or prospecting work and its qualification gaps.</p><a href={workspaceHash({ surface: 'opportunities', filters: { account: detail.account.id, lane: organization.mode === 'CUSTOMER' ? 'CUSTOMER_EXPANSION' : 'PROSPECT' }, returnTo: location })}>View opportunities for {name}</a></CustomerSection>
    <ProfilePage tab="Commercial"><ProfileCommercial key={detail.account.id} detail={detail} onWorkChanged={() => setWorkRevision(n => n + 1)} /></ProfilePage>
    <CrmContacts detail={detail} />
    <CustomerActions key={detail.account.id} accountId={detail.account.id} refreshVersion={workRevision} />
    <ProfilePage tab="More"><SupportingEvidence count={detail.missingness.length + detail.public_contacts.length + detail.public_facilities.length + (primaryBrief?.evidence_ids.length ?? 0)} investigationKey={`organization:${detail.account.id}:${primaryBrief?.assessment_id ?? 'none'}:${primaryBrief?.assessment_version ?? 0}`}>
      ...
      <section><h3>Related BTX records</h3><Commercial360 detail={detail} /><WorkbookFields key={`workbook:${detail.account.id}`} accountId={detail.account.id} />{detail.commercial_briefing && <CommercialRecords key={detail.account.id} accountId={detail.account.id} />}</section>
      ...
      <section><h3>Score details</h3>{fit?.applicable ? <><p>Prospect Fit {fitDisplay ?? 'unavailable'} · {Number(fit.coverage) * 100}% Data Coverage. Coverage does not increase opportunity quality.</p><div className="factor-list">{fit.factors.map(factor => <div className="factor-row" key={factor.name}><span><strong>{factor.label}</strong><small>{factor.reason}</small></span><strong>{factor.points ?? '—'}</strong></div>)}</div></> : <ProfileHealth health={detail.customer_health} name={name} />}</section>
    ...
    </SupportingEvidence></ProfilePage></div></ProfileTabContext.Provider>
  </section></div>
}
