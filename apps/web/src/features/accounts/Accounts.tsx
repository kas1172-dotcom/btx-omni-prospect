...
import { partnershipOptions } from '../../components/filterModel'
...
import { Button, Disclosure, Empty, EvidenceSource, MetadataRow, Notice, Panel, SearchInput, SelectInput, State, StatTile, StatusBadge, TextInput } from '../../components/UI'
...
function Portfolio({ accounts, initialSnapshot, onSnapshot, onSelect, onOmniContext }: { accounts: Account[]; initialSnapshot?: PortfolioSnapshot; onSnapshot?: (snapshot: PortfolioSnapshot) => void; onSelect: (id: string) => void; onOmniContext: (context: Pick<OmniContext, 'active_filters' | 'visible_record_ids'>) => void }) {
  ...
  const clearFilters = () => { setQuery(''); setScope('ALL'); setIndustry('ALL'); setEntity('ALL'); setTop100(false); setPartnershipScope('ALL'); setShortlistOnly(false) }
  const applied: AppliedFilter[] = [
    ...(query ? [{ key: 'query', label: `Search: ${query}`, remove: () => setQuery('') }] : []),
    ...(scope === 'RICH' ? [{ key: 'scope', label: 'Curated scenarios', remove: () => setScope('ALL') }] : []),
    ...(industry !== 'ALL' ? [{ key: 'industry', label: `Industry: ${industry}`, remove: () => setIndustry('ALL') }] : []),
    ...(entity !== 'ALL' ? [{ key: 'entity', label: `Classification: ${entity}`, remove: () => setEntity('ALL') }] : []),
    ...(top100 ? [{ key: 'top100', label: 'BTX Top 100', remove: () => setTop100(false) }] : []),
    ...(partnershipScope !== 'ALL' ? [{ key: 'partnership', label: partnershipOptions.find(([value]) => value === partnershipScope)![1], remove: () => setPartnershipScope('ALL') }] : []),
    ...(shortlistOnly ? [{ key: 'shortlist', label: 'My shortlist', remove: () => setShortlistOnly(false) }] : []),
  ]
  ...
  return <div className="surface accounts-surface">
    ...
    <Panel className="portfolio-panel" title="Customer Portfolio" action={<span className="panel-kicker">{shown.length} results</span>}>
      <FilterBar label="Portfolio filters" search={<SearchInput aria-label="Search Customers and Prospects" placeholder="Search Customer, industry, or location" value={query} onChange={event => setQuery(event.target.value)} />} sort={<SelectInput className="portfolio-sort-select" aria-label="Sort Customers and Prospects" value={`${sortKey}:${sortDirection}`} onChange={event => { const [key, order] = event.target.value.split(':') as [SortKey, SortDirection]; setSortKey(key); setSortDirection(order) }}><option value="name:ascending">Name A–Z</option><option value="name:descending">Name Z–A</option><option value="attractiveness:descending">Attractiveness high–low</option><option value="attractiveness:ascending">Attractiveness low–high</option><option value="priority:descending">Priority high–low</option><option value="industry:ascending">Industry A–Z</option></SelectInput>} filters={applied} onClear={clearFilters} count={shown.length}>
        <SelectInput label="Customer scope" aria-label="Customer scope" value={scope} onChange={event => setScope(event.target.value as Scope)}><option value="RICH">Curated scenarios</option><option value="ALL">All researched Customers and Prospects</option></SelectInput>
        <SelectInput label="Classification" aria-label="Classification" value={entity} onChange={event => setEntity(event.target.value as Classification)}><option value="ALL">All classifications</option>{[...classifications].map(value => <option key={value} value={value}>{value === 'UNAVAILABLE' ? 'Classification unavailable' : humanize(value.toLowerCase())}</option>)}</SelectInput>
        <SelectInput label="Industry" aria-label="Industry" value={industry} onChange={event => setIndustry(event.target.value)}><option value="ALL">All industries</option>{industries.map(value => <option key={value}>{value}</option>)}</SelectInput>
        <SelectInput label="Reference classification" aria-label="Reference classification" value={top100 ? 'TOP100' : 'ALL'} onChange={event => { setTop100(event.target.value === 'TOP100'); setScope('ALL') }}><option value="ALL">All reference classifications</option><option value="TOP100">BTX Top 100</option></SelectInput>
        <SelectInput label="Strategic partnerships" aria-label="Strategic partnerships" value={partnershipScope} onChange={event => setPartnershipScope(event.target.value as PartnershipScope)}>{partnershipOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</SelectInput>
        <SelectInput label="Saved planning" aria-label="Saved planning" value={shortlistOnly ? 'SAVED' : 'ALL'} onChange={event => setShortlistOnly(event.target.value === 'SAVED')}><option value="ALL">All planning</option><option value="SAVED">My growth &amp; research shortlist</option></SelectInput>
      </FilterBar>
      ...
      {shown.length === 0 && <Empty>No Customers or Prospects match the current search and filters. <Button onClick={clearFilters}>Clear filters</Button></Empty>}
    </Panel>
  </div>
}
...
function RelationshipIntelligence({ accountId, federal, location, onLocationChange, onOmniContext }: { accountId: string; federal?: { assessment: FederalAssessment; selection: OmniFederalSelection }; location: WorkspaceLocation; onLocationChange: (next: WorkspaceLocation, mode?: 'push' | 'replace') => void; onOmniContext: (context: Pick<OmniContext, 'relationship_selection'>) => void }) {
  ...
  return <div className="relationship-content">
    ...
    <div className="relationship-intelligence-header"><div><span className="eyebrow">How this organization is connected</span><p>Governed paths explain the connection, evidence, business relevance, and a bounded next review.</p></div>{projection && <div className="relationship-counts"><span><strong>{projection.validated_count}</strong> validated</span><span><strong>{projection.needs_validation_count}</strong> to review</span></div>}</div>
    ...
    {best ? <section className="relationship-decision-summary" aria-label="Best governed relationship route"><span className="eyebrow">Best governed route</span><SellerRelationshipCard path={best} /></section> : relationships && <Empty>No eligible recorded route is currently available. This does not establish a real-world absence.</Empty>}
...
function CustomerSection({ title, summary, children, defaultOpen = false, className = '' }: { title: string; summary?: string; children: ReactNode; defaultOpen?: boolean; className?: string }) { const displaySummary = summary ? presentationLabel(summary) : undefined; const initiallyOpen = className.includes('account-workspace-relationship') ? true : className.includes('attention-section') ? false : defaultOpen; return <div className={`customer-section ${className}`}><Disclosure title={<span>{title}{displaySummary && <small>{displaySummary}</small>}</span>} defaultOpen={initiallyOpen}>{children}</Disclosure></div> }
...
function CustomerDetail({ accounts, detail, initialAssessment, initialFederal, onSelect, onBack, onOmniContext, location, onLocationChange }: { accounts: Account[]; detail: Account360; initialAssessment?: OmniAssessmentSelection; initialFederal?: OmniFederalSelection; onSelect: (id: string) => void; onBack: () => void; onOmniContext: (context: Pick<OmniContext, 'selected_assessment' | 'selected_federal_opportunity' | 'relationship_selection'>) => void; location: WorkspaceLocation; onLocationChange: (next: WorkspaceLocation, mode?: 'push' | 'replace') => void }) {
  ...
  return <div className="surface account-detail-surface"><div className="account-workspace-navigation"><Button className="account-workspace-back" variant="ghost" onClick={onBack}>← Customers &amp; Prospects</Button><div className="account-switcher"><SearchInput aria-label="Switch organization" placeholder="Search Customer, Prospect, industry, or location" value={switchQuery} onChange={event => setSwitchQuery(event.target.value)} />{switchResults.length > 0 && <div className="account-switch-results" role="listbox" aria-label="Organization switcher results">{switchResults.map(item => <button type="button" role="option" aria-selected="false" className="account-row account-switch-result" key={item.id} onClick={() => choose(item.id)}><span><strong>{accountName(item)}</strong><small>{item.industries.join(' / ') || 'Industry unavailable'}</small></span><StatusBadge value={classificationLabel(item)} kind="entity" /></button>)}</div>}</div></div><section className="account-workspace" aria-label={`${name} ${organization.title} workspace`}>
    ...
    <section className="account-decision-zone" aria-label="Organization decision summary"><div className="account-decision-narrative"><article><span className="eyebrow">What changed</span><p>{primaryBrief?.what_happened ?? detail.reason_for_attention ?? 'No current governed change is recorded.'}</p></article><article><span className="eyebrow">Why it matters</span><p>{primaryBrief?.why_it_may_matter ?? detail.reason_for_attention ?? 'No supported commercial implication is currently available.'}</p></article><article><span className="eyebrow">Material uncertainty</span><p>{materialUncertainty}</p></article><article className="account-next-action"><span className="eyebrow">Governed next action</span><p>{governedAction}</p></article>{detail.alerts.length > 0 && <Notice tone="warning" title="Execution attention">{detail.alerts[0].trigger_reason} {detail.alerts[0].recommended_action}</Notice>}</div><ScoreSummary model={decisionScore} /><StatTile label="Research priority" value={detail.prospect_research_priority ? humanize(detail.prospect_research_priority) : 'Unavailable'} detail="Governed research priority" /></section>
    ...
    {Boolean(detail.federal_opportunities?.length) && <CustomerSection title={organization.mode === 'PROSPECT' ? 'Federal demand connected to this prospect' : 'Opportunities to bring to this customer'} summary={`${detail.federal_opportunities?.length ?? 0} governed routes`}><div className="card-list">{detail.federal_opportunities?.map(item => { const route = item.account_routes?.[0] ?? item.recommended_route; const selection = route ? { opportunity_id: item.opportunity_id, assessment_id: item.assessment_id, assessment_version: item.assessment_version, route_type: route.route_type, account_id: route.account_id, partnership_id: route.route_type === 'STRATEGIC_PARTNER' ? route.account_id : undefined } satisfies OmniFederalSelection : undefined; return <article className="card" key={item.assessment_id}><StatusBadge value={item.stage.label} /><h3>{item.technical.requirement}</h3><p>{route?.why}</p><strong>Next: {route?.governed_action}</strong>{selection && <Button onClick={() => setSelectedFederal(selection)}>{selectedFederal?.assessment_id === item.assessment_id ? 'Selected for Omni and relationships' : 'Use this opportunity context'}</Button>}<SupportingEvidence count={item.supporting_evidence_count} investigationKey={`account-federal:${detail.account.id}:${item.assessment_id}:${item.assessment_version}`}><p>{item.stage.explanation}</p><p>{item.durability.explanation}</p><ul>{item.technical.remaining_unknowns.map(gap => <li key={gap}>{gap}</li>)}</ul></SupportingEvidence></article>})}</div></CustomerSection>}
    <div className="account-secondary-grid"><CrmContacts detail={detail} /><CustomerSection className="account-workspace-relationship" title="People and relationship paths" summary="Governed connections"><RelationshipIntelligence key={detail.account.id} accountId={detail.account.id} federal={selectedFederal ? (() => { const assessment = detail.federal_opportunities?.find(item => item.assessment_id === selectedFederal.assessment_id && item.assessment_version === selectedFederal.assessment_version); return assessment ? { assessment, selection: selectedFederal } : undefined })() : undefined} location={location} onLocationChange={onLocationChange} onOmniContext={handleRelationshipContext} /></CustomerSection><CustomerSection className="attention-section" title="Decision panel" summary={`${detail.alerts.length} open alerts`} defaultOpen>{detail.alerts.length ? <div className="card-list">{detail.alerts.map(alert => <div className="workspace-attention" key={alertRenderKey(alert)}><State value={alert.severity} /><strong>{humanize(alert.type)}</strong><p>{alert.trigger_reason}</p><small>{alert.recommended_action}</small></div>)}</div> : <Empty>No governed alert is currently open for this organization.</Empty>}{organization.mode !== 'PROSPECT' && <Commercial360 detail={detail} />}</CustomerSection><CustomerSection title="Recent intelligence" summary={`${briefs.length} assessments`}><Signals items={detail.customer_360.intelligence} selectedAssessmentId={selectedAssessment?.assessment_id} onUseInOmni={setSelectedAssessment} /></CustomerSection></div>
    ...
    <SupportingEvidence count={detail.missingness.length + detail.public_contacts.length + detail.public_facilities.length + (primaryBrief?.evidence_ids.length ?? 0)} investigationKey={`organization:${detail.account.id}:${primaryBrief?.assessment_id ?? 'none'}:${primaryBrief?.assessment_version ?? 0}`}>
      ...
      <section><h3>How this was determined</h3><p>{organization.classification_basis}</p><GovernedExplanationDisclosure title="Decision explanation" explanation={detail.governed_explanation} /></section>
      ...
      <section><h3>Score details</h3>{fit?.applicable ? <><p>Prospect Fit {fitDisplay ?? 'unavailable'} · {Number(fit.coverage) * 100}% input coverage. Coverage does not increase opportunity quality.</p><div className="factor-list">{fit.factors.map(factor => <div className="factor-row" key={factor.name}><span><strong>{factor.label}</strong><small>{factor.reason}</small></span><strong>{factor.points ?? '—'}</strong></div>)}</div></> : <><p>Opportunity Priority {attractiveness.score ?? 'unavailable'} · {Number(attractiveness.coverage) * 100}% input coverage.</p><div className="factor-list">{availableFactors.map(factor => <div className="factor-row" key={factor.name}><span><strong>{humanize(factor.name)}</strong><small>{factor.gaps.join(', ') || 'Supported input'}</small></span><strong>{factor.score ?? '—'}</strong></div>)}</div></>}</section>
