...
function CommercialSource({ label, state }: { label: string; state: Account360['commercial_source_states']['crm'] }) { const message = state.source_state === 'AVAILABLE' ? `${label} context available` : state.source_state === 'NO_LINKED_DATA' ? `No ${label} record is linked to this canonical Customer.` : state.source_state === 'NOT_CONFIGURED' ? `${label} integration is not configured.` : `${label} context is currently unavailable.`; return <MetadataRow label={label} value={`${state.data_mode} · ${message}`} /> }
function CommercialContext({ detail }: { detail: Account360 }) { const states = detail.commercial_source_states; const paperlessAvailable = states.paperless.source_state === 'AVAILABLE'; return <CustomerSection title="Commercial context" summary={states.commercial.data_mode}><Notice title="Simulated BTX commercial context">Quotes, ownership, matching, and workflow context are POC simulation—not BTX-connected records.</Notice><CommercialSource label="Commercial" state={states.commercial} /><CommercialSource label="Paperless" state={states.paperless} /><CommercialSource label="CRM" state={states.crm} /><CommercialSource label="Orders" state={states.orders} />{paperlessAvailable && detail.paperless_quotes.length > 0 && <div className="card-list">{detail.paperless_quotes.map(quote => <div className="line" key={quote.id}><span><strong>{quote.id}</strong><small>Quoted {quote.quoted_at}</small></span><State value={quote.status} /></div>)}</div>}</CustomerSection> }
