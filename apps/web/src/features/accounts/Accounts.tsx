...
function Signals({ items }: { items: Signal[] }) { return items.length ? <div className="evidence-list">{items.map(item => <article key={item.id}><EvidenceSource title={item.business_briefing?.headline ?? item.title} source={item.source_tier ?? 'Public source'} date={item.observed_at} evidenceState={item.evidence_state} validationState={item.source_validation_state} url={item.source_url} detail={item.business_briefing?.why_it_may_matter ?? item.relevance_explanation} />{item.business_briefing && <p><strong>Next:</strong> {item.business_briefing.recommended_action ?? 'Informational only; no seller action is established.'}</p>}</article>)}</div> : <Empty>No current public Intelligence assessment is available.</Empty> }
