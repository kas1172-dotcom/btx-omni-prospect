  ...
  return <div className="surface accounts-surface">
    ...
    <Panel className="portfolio-panel" title="Customer Portfolio" action={<span className="panel-kicker">{shown.length} results</span>}>
      <FilterBar label="Portfolio filters" search={<SearchInput aria-label="Search Customers and Prospects" placeholder="Search Customer, industry, or location" value={query} onChange={event => setQuery(event.target.value)} />} sort={<SelectInput className="portfolio-sort-select" aria-label="Sort Customers and Prospects" value={`${sortKey}:${sortDirection}`} onChange={event => { const [key, order] = event.target.value.split(':') as [SortKey, SortDirection]; setSortKey(key); setSortDirection(order) }}><option value="name:ascending">Name A–Z</option><option value="name:descending">Name Z–A</option><option value="attractiveness:descending">Attractiveness high–low</option><option value="attractiveness:ascending">Attractiveness low–high</option><option value="priority:descending">Priority high–low</option><option value="industry:ascending">Industry A–Z</option></SelectInput>} filters={applied} onClear={clearFilters} count={shown.length}>
        <SelectInput label="Customer scope" value={scope} onChange={event => setScope(event.target.value as Scope)}><option value="RICH">Curated scenarios</option><option value="ALL">All researched Customers and Prospects</option></SelectInput>
        <SelectInput label="Classification" value={entity} onChange={event => setEntity(event.target.value as Classification)}><option value="ALL">All classifications</option>{[...classifications].map(value => <option key={value} value={value}>{value === 'UNAVAILABLE' ? 'Classification unavailable' : humanize(value.toLowerCase())}</option>)}</SelectInput>
        <SelectInput label="Industry" value={industry} onChange={event => setIndustry(event.target.value)}><option value="ALL">All industries</option>{industries.map(value => <option key={value}>{value}</option>)}</SelectInput>
        <SelectInput label="Reference classification" value={top100 ? 'TOP100' : 'ALL'} onChange={event => { setTop100(event.target.value === 'TOP100'); setScope('ALL') }}><option value="ALL">All reference classifications</option><option value="TOP100">BTX Top 100</option></SelectInput>
        <SelectInput label="Strategic partnerships" value={partnershipScope} onChange={event => setPartnershipScope(event.target.value as PartnershipScope)}>{partnershipOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</SelectInput>
        <SelectInput label="Saved planning" value={shortlistOnly ? 'SAVED' : 'ALL'} onChange={event => setShortlistOnly(event.target.value === 'SAVED')}><option value="ALL">All planning</option><option value="SAVED">My growth &amp; research shortlist</option></SelectInput>
      </FilterBar>
