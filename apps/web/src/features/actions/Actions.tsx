...
export function Actions({ items, suggestions, principal, accounts, signals, warning, onItem, onSuggestions, onAccount, onActionSelect, onOmniContext, sourceAlertId, onClearSource, location, onLocationChange }: Props) {
  ...
  return <div className={`surface actions-surface ${selected && tab !== 'suggestions' ? 'detail-open' : ''}`}>
    ...
    {tab === 'suggestions' ? <>{sourceAlertId && <section className="notice" aria-label="Selected Today priority"><p>Showing the recommendation linked to your Today priority.</p><Button onClick={onClearSource}>Show all suggestions</Button>{scopedSuggestions.filter(item => item.converted_action_id).map(item => <Button key={item.id} onClick={() => { onClearSource(); onLocationChange({ ...location, subview: 'actions', actionId: item.converted_action_id }) }}>Open existing action</Button>)}</section>}<SuggestionList onClear={clear} suggestions={scopedSuggestions} name={customerName} onConvert={convert} onFeedback={(id, patch) => onSuggestions(suggestions.map(item => item.id === id ? { ...item, ...patch } : item))} onRefreshed={onSuggestions} query={query} onQuery={value => changeFilters({ query: value })} priority={priority} onPriority={value => changeFilters({ priority: value })} sort={sort} onSort={value => onLocationChange({ ...location, sort: value }, 'replace')} view={suggestionView} onView={value => changeFilters({ suggestion_view: value })} page={page} onPage={value => changeFilters({ page: String(value) })} selectedId={location.recordId} onSelected={recordId => onLocationChange({ ...location, recordId }, 'replace')} /></> : <>
