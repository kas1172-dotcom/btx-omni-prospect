...
for (const viewport of [{ width: 1440, height: 900 }, { width: 390, height: 844 }]) {
  test(`v2 account scoring, disclosure and unknown pursuit inputs at ${viewport.width}`, async ({ page }, testInfo) => {
    ...
    const disclosure = health.getByRole('button', { name: 'Why this?', exact: true })
    await expect(disclosure).toHaveAttribute('aria-expanded', 'false')
    await disclosure.click()
    await expect(page.getByRole('dialog', { name: 'Why this?' })).toContainText('Calculated result: 72.50/100')
    await expect(page.getByRole('dialog', { name: 'Why this?' })).toContainText('Factors driving this result')
