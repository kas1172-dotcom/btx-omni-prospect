...
for (const viewport of [{ width: 1440, height: 900 }, { width: 390, height: 844 }]) {
  test(`v2 account scoring, disclosure and unknown pursuit inputs at ${viewport.width}`, async ({ page }, testInfo) => {
    ...
    const disclosure = health.getByRole('button', { name: 'Formula, inputs and supporting evidence' })
    await expect(disclosure).toHaveAttribute('aria-expanded', 'false')
    await disclosure.click()
    await expect(health).toContainText('Calculated result: 72.50/100')
    await expect(health).toContainText('Factors driving this result')
