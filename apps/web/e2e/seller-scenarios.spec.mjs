...
test('Tactical Map composes canonical industry and SAMPLE commercial segment filters', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'All researched Customers and Prospects' }).click()
  await page.getByRole('button', { name: 'Defense', exact: true }).click()
  await page.getByRole('button', { name: 'Customer', exact: true }).click()
  await expect(page.locator('.map-toolbar-status')).toHaveText('All researched · Defense · Customer')
  await page.waitForTimeout(2200)
  await expect(page.getByRole('button', { name: 'Map Customer marker: Lockheed Martin' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Map Customer marker: Anduril Industries' })).toHaveCount(0)
  ...
  await expect(page.getByRole('button', { name: 'Map Customer marker: Lockheed Martin' })).toHaveCount(0)
  ...
  await expect(page.locator('.map-stage-status')).toContainText('0 mapped Customers and Prospects')
  await expect(page.getByText('No matching mapped records exist for the selected market, coverage, and Customer relationship filters.')).toBeVisible()
  await page.getByRole('button', { name: 'Clear filters' }).click()
  await expect(page.locator('.map-toolbar-status')).toHaveText('Curated scenarios · All')
  await expect(page.getByRole('button', { name: 'Map Customer marker: Anduril Industries' })).toBeVisible()
  ...
  await expect(page.locator('.map-stage-status')).toContainText('1 mapped Customers and Prospects')
  const applied = page.getByRole('button', { name: 'Map Customer marker: Applied Materials' })
