...
test('seller saves a dated private research shortlist and filters the same portfolio', async ({ page }) => {
  ...
  await page.getByText('Strategic partner & shortlist', { exact: true }).click()
  ...
  await page.getByRole('checkbox', { name: 'My growth & research shortlist' }).check()
...
test('manager creates an audited partnership designation and portfolio include/exclude filters agree', async ({ page }) => {
  ...
  await expect(page.getByText('Strategic partner: Yes', { exact: true })).toBeVisible()
  ...
  await page.getByRole('button', { name: 'Remove partnership filter' }).click()
  ...
  await expect(page.getByText('Strategic partner: Yes', { exact: true })).toHaveCount(0)
})
