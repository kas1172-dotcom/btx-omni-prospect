...
for (const width of [390, 1440]) {
  test(`Today keeps action and validation outcomes distinct at ${width}px`, async ({ page }, testInfo) => {
    ...
    await page.getByRole('button', { name: 'View supporting evidence (1)' }).click()
    await page.getByRole('button', { name: 'Program, components and possible BTX fit' }).click()
    await expect(page.getByText('Candidate fit 8 requires validation.')).toBeVisible()
    if (width === 390) await page.getByRole('button', { name: 'Close supporting evidence' }).click()
