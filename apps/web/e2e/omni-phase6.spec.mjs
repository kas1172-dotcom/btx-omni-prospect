...
test('Omni trigger docks safely after dragging and preserves normal activation', async ({ page }) => {
  await page.goto('/')
  const trigger = page.getByLabel('Open Omni assistant')
  ...
  await page.mouse.up()
  await expect(page.getByRole('dialog', { name: 'Omni' })).toBeHidden()
  const docked = await trigger.boundingBox()
  expect(docked.x + docked.width).toBeGreaterThan(page.viewportSize().width - 24)
  await page.reload()
  await expect(trigger).toBeVisible()
  const persisted = await trigger.boundingBox()
  expect(persisted.x + persisted.width).toBeGreaterThan(page.viewportSize().width - 24)
  await trigger.focus()
