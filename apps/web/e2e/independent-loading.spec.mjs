...
test('map filters and a canonical selected site survive Account360 navigation', async ({ page }) => {
  ...
  await page.getByRole('group', { name: 'Industry', exact: true }).getByRole('button', { name: 'Semiconductor', exact: true }).click()
  ...
  await selected.getByRole('button', { name: 'View Customer 360', exact: true }).click()
