...
for (const viewport of [{ name: 'desktop', width: 1440, height: 900 }, { name: 'mobile', width: 390, height: 844 }]) {
  test(`Wave 3 Organization 360 and relationship decision hierarchy · ${viewport.name}`, async ({ page }, testInfo) => {
    ...
    await expect(decision).toContainText('Why it may matter')
    await expect(decision).toContainText('Still unconfirmed')
    await expect(decision).toContainText('Next decision')
    await decision.getByRole('button', { name: 'Why this?', exact: true }).click()
    ...
    await relationship.getByRole('button', { name: /People and relationship paths/ }).click()
    await expect(relationship.getByText('Best supported route')).toBeVisible()
