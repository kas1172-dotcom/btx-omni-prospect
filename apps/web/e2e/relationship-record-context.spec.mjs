...
for (const width of [390, 1440]) {
  test(`commercial record context preserves ranked authority and opens actual evidence at ${width}`, async ({ page }, testInfo) => {
    ...
    const table = section.locator('details').filter({ has: page.getByText('Visible assertions · keyboard and text equivalent', { exact: true }) })
    await table.locator('summary').click()
    await table.getByRole('button').filter({ hasText: record.id }).click()
