...
test('Suggestions remain distinct and convert idempotently without an external write', async ({ page }) => {
  ...
  const conversion = page.locator('.suggestion-detail').getByRole('button', { name: 'Create Action' }).first()
...
test('approval is Manager-only and remains separate from work status', async ({ browser }) => {
  ...
  await expect(managerPage.locator('.action-detail').getByText('Assigned seller', { exact: true })).toBeVisible()
  await managerPage.getByRole('button', { name: 'Approve' }).click()
  await expect(managerPage.getByText('Approved by reviewer', { exact: true })).toBeVisible()
  ...
  await expect(managerPage.getByRole('list', { name: 'Action history' }).getByRole('listitem').filter({ hasText: 'Status available in supporting details' })).toBeVisible()
