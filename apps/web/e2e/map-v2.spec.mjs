...
test('Map V2 is marker-first and opens canonical Customer context', async ({ page }) => {
  ...
  const customer = page.getByRole('button', { name: 'Customer marker: Lockheed Martin', exact: true }).first()
  ...
  await expect(page.getByRole('button', { name: /30-mile straight-line radius/ })).toBeVisible()
...
for (const width of [390, 320]) test(`Map V2 mobile sheets remain reachable at ${width}px`, async ({ page }) => {
  ...
  await sheet.getByRole('button', { name: 'Intelligence', exact: true }).click()
  await sheet.getByRole('button', { name: 'Apply to map' }).click()
  await page.getByRole('button', { name: 'List', exact: true }).click()
  const customer = page.getByRole('region', { name: 'Map results', exact: true }).getByRole('button').filter({ hasText: /customer site · inspect details/i }).first()
  await customer.click()
