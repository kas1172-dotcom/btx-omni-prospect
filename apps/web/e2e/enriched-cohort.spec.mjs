...
test('all eleven persisted enriched accounts have distinct briefings and twelve-month records in the seller UI', async ({ page }, testInfo) => {
  ...
  for (const id of accounts) {
    ...
    await expect(page.locator('.account-why')).toContainText(account.commercial_briefing.summary)
    await expect(page.locator('.account-why')).toContainText(account.commercial_briefing.next_action)
...
for (const [width, height] of [[360, 800], [390, 844], [768, 1024], [1440, 900], [1920, 1080]]) {
  test(`enriched account reading is contained at ${width}x${height}`, async ({ page }, testInfo) => {
    ...
    await expect(page.locator('.account-why')).toContainText('New cleaning and inspection scope changed the current quote')
