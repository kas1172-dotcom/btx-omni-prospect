...
for (const width of [390, 1440]) {
  test(`exact CRM proposal approval and uncertain sample retry persist at ${width}`, async ({ page }, testInfo) => {
    ...
    await expect(panel.getByText('Proposal approval: APPROVED', { exact: true })).toBeVisible()
    let lost = false
    ...
    await panel.getByRole('button', { name: 'Run approved local sample', exact: true }).click()
    ...
    await expect(panel.getByRole('button', { name: 'Run approved local sample', exact: true })).toBeDisabled()
    ...
    await page.locator('.action-row').filter({ hasText: title }).click()
    await panel.locator(':scope > summary').click()
    await expect(panel).toContainText('Local sample completed — not a HubSpot write')
