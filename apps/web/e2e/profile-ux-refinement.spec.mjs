...
for (const width of [1440, 390]) test(`profile tabs, score explanation and investigation context at ${width}`, async ({ page }) => {
  ...
  await expect(page.locator('.profile-health-kpi')).not.toContainText('Watch')
  const why = page.locator('.profile-health-kpi').getByRole('button', { name: 'Why this?' })
  await why.click()
  ...
  await expect(tabs.getByRole('tab', { name: 'Opportunities', exact: true })).toHaveCount(0)
  await expect(page.getByRole('link', { name: 'View opportunities for Honeywell' }).first()).toBeVisible()
  await page.locator('.profile-planning-popover > summary').click()
