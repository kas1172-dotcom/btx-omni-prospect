...
| National commercial graph | Map | Market/facility read model | Researched canonical accounts, public facilities, BTX facilities | Yes | Facility and market feeds | Required | Renders only loaded researched locations; primary markets are Aerospace, Defense, Semiconductor, Space Exploration, Energy, and Medical. Proximity informs seller planning only. | Additional verified facility data |
