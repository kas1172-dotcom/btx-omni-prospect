...
DEFINITIONS = (
    ...
    {"name": "read_decision", "description": "Exact factor trace for one family and subject_id listed by read_decisions. No other account scope is allowed.", "arguments": ["family", "subject_id"]},
    {"name": "read_rubric_example", "description": "One explicitly fictional what-if receipt listed in read_history. Required example_id; never treat it as measured customer data.", "arguments": ["example_id"]},
...
def _decision_index(value):
    keys = ('family', 'subject_id', 'score', 'score_range', 'weighted_score', 'weighted_band', 'band',
            'rule_version', 'status', 'eligible', 'priority_rank', 'priority_class')
    return {**{key: value[key] for key in keys if key in value}, 'factor_details_omitted': True,
            'detail_tool': 'read_decision'}


def model_payload(name, value):
    """Page large evidence collections without raising the model's hard budgets."""
    if name == 'read_history':
        case = value.get('case_briefing', {})
        lab = case.get('rubric_examples', {})
        if lab:
            index = {key: {'label': row.get('label', 'Fictional arithmetic exercise'),
                          'detail_tool': 'read_rubric_example', 'detail_arguments': {'example_id': key}}
                     for key, row in lab.get('examples', {}).items()}
            return {**value, 'case_briefing': {**case, 'rubric_examples': {**lab, 'examples': index,
                    'detail_policy': 'Index only. Retrieve one full calculated receipt; these are not current customer facts.'}}}
    if name == 'read_decisions':
        return {**value, 'opportunities': [
            {key: _decision_index(item) if isinstance(item, dict) and 'decision_id' in item else item
             for key, item in row.items() if key != 'attractiveness'} for row in value.get('opportunities', ())],
            'action_priorities': [{key: _decision_index(item) if isinstance(item, dict) and 'decision_id' in item else item
                                  for key, item in row.items()} for row in value.get('action_priorities', ())],
            'detail_policy': 'Opportunity/action factor details are indexed, not discarded. The duplicate legacy attractiveness alias is omitted. Call read_decision with the listed family and subject_id.'}
    return value


class CommercialToolSession:
    """Scope is server-resolved once; the model cannot select another account or owner."""
    def __init__(self, sample, account_id, *, work_items=(), selected_relationship=None, selected_market=None, signal_briefs=()):
        ...
        self.signal_briefs = tuple(signal_briefs)
    ...
    def read(self, name, arguments):
        ...
        ledger = self.ledger
        if name == 'read_rubric_example':
            example = ledger.get('commercial_case', {}).get('rubric_examples', {}).get('examples', {}).get(arguments['example_id'])
            if example is None:
                raise ValueError('No such example in this account scope.')
            return {'label': 'Fictional what-if, not current customer facts', 'synthetic': True, 'data_mode': 'SAMPLE',
                    'example_id': arguments['example_id'], 'receipt': example}
        if name == 'read_decision':
            def find(value):
                if isinstance(value, dict):
                    if value.get('family') == arguments['family'] and value.get('subject_id') == arguments['subject_id'] and 'decision_id' in value:
                        return value
                    ...
                    return None
                return next((found for child in children if (found := find(child)) is not None), None)
            result = find(self.read('read_decisions', {}))
            if result is None:
                raise ValueError('Decision family/subject is not in this account scope.')
            return result
        if name == "read_history":
        ...
        if name == "read_decisions":
            ...
            result.update(customer_risk_projection(account_id=self.account_id,
                current_customer=account.relationship.value in {'CURRENT_CUSTOMER', 'FORMER_CUSTOMER'},
                internal_decision=result['internal_commercial_risk'], signal_briefs=self.signal_briefs,
            ...
            def bounded(value):
                if isinstance(value, dict):
                    if "decision_id" in value:
                        return {k: bounded(value[k]) for k in ("decision_id", "family", "subject_id", "score", "score_range", "configuration_version", "rule_version", "weighted_score", "weighted_band", "band", "what_would_change_result", "calculation_trace", "status", "priority_rank", "priority_class", "eligibility_reasons", "blocking_constraints", "factors", "data_coverage") if k in value}
        ...
        for index in range(max_calls):
            ...
            try:
                ...
                if len(encoded) > 24000:
                    value = model_payload(selection['tool'], value)
                    encoded = json.dumps(value, default=str, sort_keys=True)
