...
from btx_omni.domain.accounts import AccountFacility, AccountRelationship, CanonicalAccount
...
from btx_omni.modules.matching.commercial import CommercialComponent, HistoricalQuoteContext
...
from btx_omni.providers.research.btx_profile import load_btx_business_units
...
from btx_omni.providers.research.ingestion import ResearchAccount, apply_facility_feed_enrichment, build_researched_canonical_accounts, load_usaspending_recipient_identities
...
def build_sample_environment() -> SampleEnvironment:
    ...
    public_signals = tuple(PublicScenarioSignal(scenario.event.source_id, account_id, scenario.event.source_url, accounts_by_id[account_id].industries[0], accounts_by_id[account_id].provenance) for account_id, scenario in rich_scenarios.items())
