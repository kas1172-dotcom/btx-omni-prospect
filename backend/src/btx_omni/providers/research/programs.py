...
def load_programs(*, account_ids: set[str]) -> tuple[Program, ...]:
    ...
    for row in payload["programs"]:
        ...
        result.append(Program(row["id"], prime_account_id, row["name"], row.get("industry"), EvidenceState.CONFIRMED, source_provenance(row, classification=Classification.PUBLIC)))
