...
class DurableCanonicalProgramRepository:
    ...
    def create_program(
        ...
        result = DurableCanonicalProgram(program, identity_key, originating_candidate_id, now)
        ...
        if matched:
            if matched.program.name == name and matched.program.account_id == canonical_account_id and matched.program.system == system:
                return matched
            raise ValueError("exact canonical Program identity collision prevents durable Program creation.")
        if any(item.program.name.casefold() == name.casefold() or item.program.id == program.id for item in existing):
            raise ValueError("exact canonical Program identity collision prevents durable Program creation.")
        with self.engine.begin() as connection:
            connection.execute(delete(durable_canonical_programs).where(durable_canonical_programs.c.id == program.id))
            connection.execute(insert(durable_canonical_programs).values(
                ...
                promoted_at=None, promotion_provenance=None,
            ))
        return result
