...
work_items = Table("work_items", metadata, Column("id", String(64), primary_key=True), Column("account_id", ForeignKey("accounts.id"), nullable=False), Column("opportunity_id", String(64)), Column("status", String(32), nullable=False), Column("summary", Text, nullable=False), Column("evidence_ids", Text, nullable=False), Column("owner_id", String(128)), Column("priority", String(16), nullable=False), Column("due_date", String(16)), Column("notes", Text), Column("idempotency_key", String(128), nullable=False, unique=True), Column("created_at", DateTime(timezone=True), nullable=False))
work_audit_events = Table("work_audit_events", metadata, Column("id", Integer, primary_key=True), Column("work_item_id", ForeignKey("work_items.id"), nullable=False), Column("event", String(32), nullable=False), Column("actor_id", String(128), nullable=False), Column("occurred_at", DateTime(timezone=True), nullable=False), Column("note", Text))
