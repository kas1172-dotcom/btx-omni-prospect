...
from btx_omni.domain.work import Action, ActionPriority, ActionStatus, ApprovalStatus
...
def test_canonical_poc_domain_contracts_are_instantiable_and_source_governed() -> None:
    ...
    configuration = ScoreConfiguration(
        "config-1", "account-attractiveness-v1", True, "sample", NOW
    )
    ...
    assert (
        identity.canonical_account_id,
        paperless.canonical_account_id,
        contact.company_id,
        deal.business_unit,
        activity.occurred_at,
        assessment.score,
        work.owner_id,
    ) == ("acct-1", "acct-1", "company-1", "southwest", NOW, Decimal(80), "owner-1")


def test_public_identity_and_simulated_commercial_context_keep_separate_provenance() -> (
    None
):
    sample = build_sample_environment()
    assert all(
        account.provenance
        and account.provenance.data_mode.value == "CONNECTED"
        and not account.provenance.synthetic
        for account in sample.accounts
    )
    assert all(
        context.provenance.data_mode.value == "SAMPLE" and context.provenance.synthetic
        for context in sample.commercial_contexts
    )
