...
async def test_omni_typed_context_is_bounded_and_backwards_compatible() -> None:
    async with AsyncClient(transport=ASGITransport(app=create_app()), base_url="http://test") as client:
        ...
        monitor_context_exact = await client.post("/api/omni", json={"question": "What Monitor context is active?", "context": {"surface": "MONITOR", "prior_turns": ""}})
    ...
    assert typed.status_code == legacy.status_code == intelligence_context.status_code == map_context.status_code == action_context.status_code == monitor_context.status_code == monitor_context_exact.status_code == monitor_global.status_code == 200
