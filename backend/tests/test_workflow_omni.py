...
SELLER = Principal("seller", "Seller", PrincipalRole.SALESPERSON)
MANAGER = Principal("manager", "Manager", PrincipalRole.MANAGER)
...
def event_id_for(sample, account_id: str) -> str:
    raw = next(
        ...
        and next(
            ...
            if account.legal_name == item.account_name
        )
        == account_id
    )
    ...
    return normalize_signal(
        raw, account_name_to_id=names, provenance=account.provenance
    ).id


def public_facility_id_for(sample, account_id: str) -> str:
    return next(
        item.id for item in sample.public_facilities if item.account_id == account_id
    )


def create_selected_work_item(
    *,
    account_id: str = "boeing",
    evidence_ids: tuple[str, ...] = ("FAA_BOEING",),
    summary: str = "Review Boeing public evidence",
):
    service = WorkService()
    item = service.create(
...
def test_governed_action_lifecycle_audit_and_idempotency() -> None:
    ...
    item = service.transition(
        first.id, WorkStatus.IN_PROGRESS, principal=SELLER, occurred_at=NOW
    )
    item = service.transition(
        item.id, WorkStatus.COMPLETED, principal=SELLER, occurred_at=NOW
    )
    assert item.status is WorkStatus.COMPLETED and len(service.audit(item.id)) == 3
    dismissed = service.create(
    ...
    assert (
        service.transition(
            dismissed.id, WorkStatus.CANCELED, principal=SELLER, occurred_at=NOW
        ).status
        is WorkStatus.CANCELED
    )
...
def test_omni_explains_alerts_coordination_matching_and_truthful_missingness() -> None:
    ...
    dormant = omni.answer(
        ...
        question="Why is this dormant account at risk?",
        observed_at=NOW,
    )
    stale = omni.answer(
        sample,
        account_id="lockheed-martin",
        question="What should we do about the quote?",
        observed_at=NOW,
    )
    cross_bu = omni.answer(
        sample, account_id="boeing", question="Who needs coordination?", observed_at=NOW
    )
    defense = omni.answer(
        sample,
        account_id="lockheed-martin",
        question="Explain the award match and score",
        observed_at=NOW,
    )
    external = omni.answer(
        sample,
        account_id="rocket-lab-usa",
    ...
    assert (
        not defense.source_of_record and "cannot perform CRM writes" in defense.content
    )
...
def test_omni_supports_grounded_unscoped_and_session_follow_up_context() -> None:
    sample = build_sample_environment()
    omni = OmniOrchestrator()
    overview = omni.answer(
        sample, account_id=None, question="What should I review today?", observed_at=NOW
    )
    follow_up = omni.answer(
        sample,
        account_id=None,
        question="Explain this score and its gaps",
        observed_at=NOW,
        context={"session_account_id": "boeing"},
    )
...
def test_omni_routes_a_selected_resolved_event_with_evidence_and_sample_boundary() -> (
    None
):
    sample = build_sample_environment()
    ...
    response = OmniOrchestrator().answer(
        sample,
        account_id=None,
        question="Why does this matter to BTX?",
        observed_at=NOW,
        context={"surface": "INTELLIGENCE", "selected_event_id": event_id},
    )
    ...
    assert response.citation_links and response.context_used == {
        "event_id": event_id,
        "surface": "INTELLIGENCE",
        "account_id": "boeing",
    }
    assert any(
        "No canonical facility association" in item for item in response.missingness
    )


def test_omni_selected_event_handles_unresolved_conflicting_evidence_without_guessing() -> (
    None
):
    sample = build_sample_environment()
    raw = RawSignal(
        "controlled-unresolved",
        SignalKind.PRESS_RELEASE,
        "Controlled unresolved public event",
        "https://example.test/unresolved",
        NOW,
        None,
        "Unmapped program",
        EvidenceState.CONFLICTING,
        "Controlled unit fixture.",
    )
    ...
    assert response.account_id == "" and response.context_used == {
        "event_id": event_id,
        "surface": "INTELLIGENCE",
    }
    assert "CONFLICTING" in response.content and any(
        "Unmapped program" in item for item in response.missingness
    )


def test_omni_selected_event_preserves_non_event_queries_and_invalid_event_handling() -> (
    None
):
    sample = build_sample_environment()
    event_id = event_id_for(sample, "boeing")
    omni = OmniOrchestrator()
    cross_account = omni.answer(
        sample,
        account_id=None,
        question="Which Defense accounts have open quotes?",
        observed_at=NOW,
        context={"surface": "INTELLIGENCE", "selected_event_id": event_id},
    )
    general = omni.answer(
    ...
    invalid = omni.answer(
        ...
        question="What happened here?",
        observed_at=NOW,
        context={"surface": "INTELLIGENCE", "selected_event_id": "missing-event"},
    )
    assert (
        "Defense researched account(s) with open quotes" in cross_account.content
        and "FAA production oversight update" not in cross_account.content
    )
    assert (
        "curated public-company universe" in general.content
        and "FAA production oversight update" not in general.content
    )
    assert (
        "not available in the current canonical Monitor read model" in invalid.content
    )


def test_omni_routes_a_selected_researched_facility_without_geographic_inference() -> (
    None
):
    sample = build_sample_environment()
...
def test_omni_routes_a_selected_btx_facility_without_customer_identity_or_sample_claim() -> (
    None
):
    sample = build_sample_environment()
    response = OmniOrchestrator().answer(
...
def test_omni_selected_facility_evidence_relevance_missing_and_conflicting_account_context() -> (
    None
):
    sample = build_sample_environment()
    facility_id = public_facility_id_for(sample, "boeing")
    ...
    assert (
        "facility score" in relevance.content
        and "current SAMPLE commercial dataset" in relevance.content
    )
...
def test_omni_selected_facility_preserves_non_facility_queries_and_invalid_ids() -> (
    None
):
    sample = build_sample_environment()
    facility_id = public_facility_id_for(sample, "boeing")
    ...
    invalid = omni.answer(
        ...
        question="Tell me about this facility.",
        observed_at=NOW,
        context={"surface": "MAP", "selected_facility_id": "missing-facility"},
    )
    assert (
        "Defense researched account(s) with open quotes" in cross_account.content
        and "Boeing headquarters" not in cross_account.content
    )
    assert (
        "curated public-company universe" in general.content
        and "Boeing headquarters" not in general.content
    )
    assert "can't resolve the selected facility" in invalid.content


def test_omni_routes_a_selected_work_item_with_stored_evidence_and_sample_boundary() -> (
    None
):
    sample = build_sample_environment()
...
def test_omni_selected_work_item_explains_linked_conflicting_evidence_and_review_scope() -> (
    None
):
    sample = build_sample_environment()
    raw = RawSignal(
        "controlled-action-conflict",
        SignalKind.PRESS_RELEASE,
        "Controlled conflicting action evidence",
        "https://example.test/action-conflict",
        NOW,
        "Boeing",
        None,
        EvidenceState.CONFLICTING,
    ...
    service, item = create_selected_work_item(
        evidence_ids=tuple(event_record["evidence_ids"]),
        summary="Review conflicting public evidence",
    )
    response = OmniOrchestrator().answer(
        controlled,
        account_id=None,
...
def test_omni_selected_work_item_handles_next_step_conflicts_invalid_and_read_only_execution() -> (
    None
):
    sample = build_sample_environment()
    service, item = create_selected_work_item()
    omni = OmniOrchestrator()
    next_step = omni.answer(
        sample,
        account_id=None,
    ...
    outcome = omni.answer(
        sample,
        account_id=None,
        question="What would happen if I act on this?",
        observed_at=NOW,
        context={"surface": "ACTIONS", "selected_action_id": item.id},
        work_items=service.list(),
    )
    before = service.list()
    execution = omni.answer(
        sample,
        account_id=None,
        question="Go ahead and execute this action.",
        observed_at=NOW,
        context={"surface": "ACTIONS", "selected_action_id": item.id},
        work_items=service.list(),
    )
    invalid = omni.answer(
        ...
        question="Why was this created?",
        observed_at=NOW,
        context={"surface": "ACTIONS", "selected_action_id": "missing-work-item"},
        work_items=service.list(),
    )
    assert (
...
def test_omni_selected_work_item_preserves_non_action_queries_and_missing_event_association() -> (
    None
):
    sample = build_sample_environment()
    service, item = create_selected_work_item()
    ...
    missing_account_service, missing_account_item = create_selected_work_item(
        account_id="unmapped-account", summary="Review unmapped action"
    )
    missing_account = omni.answer(
        sample,
        account_id=None,
        question="What account is this for?",
        observed_at=NOW,
        context={"surface": "ACTIONS", "selected_action_id": missing_account_item.id},
        work_items=missing_account_service.list(),
    )
    assert (
        "Defense researched account(s) with open quotes" in cross_account.content
        and item.summary not in cross_account.content
    )
    assert (
        "curated public-company universe" in general.content
        and item.summary not in general.content
    )
...
def test_omni_routes_selected_account_relationships_through_canonical_service() -> None:
    ...
    response = OmniOrchestrator().answer(
        ...
        context={
            ...
            "selected_account_id": "spirit-aerosystems",
        },
    )
    ...
    assert response.context_used == {
        "surface": "ACCOUNT_DETAIL",
        "account_id": "spirit-aerosystems",
    }


def test_omni_relationship_routing_preserves_source_less_and_conflicting_evidence() -> (
    None
):
    sample = build_sample_environment()
    omni = OmniOrchestrator()
    ...
    conflicting = replace(
        ...
        relationship_edges=tuple(
            replace(edge, evidence_state=EvidenceState.CONFLICTING)
    ...
    assert any(
        "no attached relationship source IDs" in item
        for item in source_less.missingness
    )
...
def test_omni_relationship_routing_uses_explicit_pair_program_and_warm_path_context() -> (
    None
):
    sample = build_sample_environment()
    omni = OmniOrchestrator()
    ...
    assert (
        pair.context_used["account_id"] == "boeing"
        and pair.context_used["related_account_id"] == "spirit-aerosystems"
    )
    assert (
        "F-35 Lightning II" in programs.content and "NASA Artemis" in programs.content
    )
    assert "Boeing has current SAMPLE commercial context" in warm.content
    assert "not a guaranteed introduction" in warm.content


def test_omni_relationship_routing_handles_contacts_no_path_invalid_and_read_only_requests() -> (
    None
):
    sample = build_sample_environment()
    omni = OmniOrchestrator()
...
def test_omni_screen_summaries_respect_today_and_accounts_visible_scope() -> None:
    ...
    work, item = create_selected_work_item()
    alert = CommercialAlertEngine().evaluate(
        sample.commercial_contexts, sample.quotes, observed_at=NOW, orders=sample.orders
    )[0]
    omni = OmniOrchestrator()
    today = omni.answer(
        sample,
        ...
        context={
            "surface": "TODAY",
            "visible_record_ids": [alert.id, event["id"], item.id],
        },
    ...
    assert (
        alert.type.value in today.content
        and event["title"] in today.content
        and item.summary in today.content
    )
...
def test_omni_screen_summaries_use_selected_and_visible_canonical_context() -> None:
    ...
    account_detail = omni.answer(
        sample,
        account_id=None,
        question="Give me the highlights from this view.",
        observed_at=NOW,
    ...
    intelligence = omni.answer(
        sample,
        account_id=None,
        question="What should I know from this screen?",
        observed_at=NOW,
        context={
            "surface": "INTELLIGENCE",
            "selected_event_id": selected_event,
            "active_filters": {"market": "Defense"},
            "visible_record_ids": [selected_event],
        },
        intelligence_events=event_records,
    )
    actions = omni.answer(
        ...
        context={
            "surface": "ACTIONS",
            "selected_action_id": item.id,
            "visible_record_ids": [item.id],
        },
        work_items=work.list(),
    )
    assert "Account Detail summary for Boeing" in account_detail.content
    assert account_detail.context_used == {
        "surface": "ACCOUNT_DETAIL",
        "account_id": "boeing",
    }
    assert (
        "Current Intelligence view" in intelligence.content
        and "Selected event:" in intelligence.content
    )
    assert intelligence.context_used == {
        "surface": "INTELLIGENCE",
        "filters": {"market": "Defense"},
        "event_id": selected_event,
    }
    assert (
        "Current Actions view" in actions.content
        and "Selected action:" in actions.content
    )
...
def test_omni_screen_summary_map_empty_invalid_and_specific_route_protection() -> None:
    ...
    invalid = omni.answer(
        sample,
        account_id=None,
        question="Summarize this screen.",
        observed_at=NOW,
        context={"surface": "ACCOUNTS", "visible_record_ids": ["unknown-account"]},
    )
    event_question = omni.answer(
        sample,
        account_id=None,
        question="Why does this matter?",
        observed_at=NOW,
        context={"surface": "INTELLIGENCE", "selected_event_id": event_id},
        intelligence_events=OmniOrchestrator._sample_event_records(sample),
    )
    action_question = omni.answer(
        sample,
        account_id=None,
        question="Why was this created?",
        observed_at=NOW,
    ...
    cross_account = omni.answer(
        ...
        question="Which Defense accounts have open quotes?",
        observed_at=NOW,
        context={"surface": "ACCOUNTS", "visible_record_ids": ["boeing"]},
    )
    general = omni.answer(
        sample,
        account_id=None,
    ...
    assert (
        "Canonical researched facility" in map_facility.content
        and map_facility.context_used["facility_id"] == facility_id
    )
...
def test_omni_cross_account_score_ranking_uses_canonical_scores_filters_and_bounds() -> (
    None
):
    sample = build_sample_environment()
    omni = OmniOrchestrator()
    ranked = omni.answer(
        sample,
        account_id=None,
        question="Which accounts have the highest attractiveness scores?",
        observed_at=NOW,
    )
...
def test_omni_cross_account_work_intelligence_intersection_and_zero_results_are_read_only() -> (
    ...
    event = next(
        item
        for item in OmniOrchestrator._sample_event_records(sample)
        if item["id"] == event_id
    )
    work, item = create_selected_work_item()
    closed = work.create(
        account_id="lockheed-martin",
        summary="Closed work",
        evidence_ids=("ev-closed",),
        idempotency_key="closed-cross-account",
        actor_id="seller",
        occurred_at=NOW,
    )
    work.transition(
        closed.id, WorkStatus.IN_PROGRESS, principal=SELLER, occurred_at=NOW
    )
    work.transition(closed.id, WorkStatus.COMPLETED, principal=SELLER, occurred_at=NOW)
    omni = OmniOrchestrator()
    before = work.list()
    actions = omni.answer(
        sample,
        account_id=None,
        question="Which accounts have open actions?",
        observed_at=NOW,
        work_items=work.list(),
    )
    intelligence = omni.answer(
        sample,
        account_id=None,
        question="Which accounts have Intelligence?",
        observed_at=NOW,
        intelligence_events=(event,),
    )
    intersection = omni.answer(
        sample,
        account_id=None,
        question="Which accounts have both recent Intelligence and open actions?",
        observed_at=NOW,
        intelligence_events=(event,),
        work_items=work.list(),
    )
    ...
    assert (
        event["title"] in intelligence.content
        and "source-backed" in intelligence.content
    )
    assert (
        "Boeing" in intersection.content
        and "does not establish" in intersection.content
    )
...
def test_omni_cross_account_quotes_comparison_and_route_protection() -> None:
    ...
    history = omni.answer(
        sample,
        account_id=None,
        question="Where do we have quote history?",
        observed_at=NOW,
    )
    comparison = omni.answer(
        sample,
        account_id=None,
        question="Compare Boeing and Lockheed Martin.",
        observed_at=NOW,
    )
    unresolved = omni.answer(
        sample,
        account_id=None,
        question="Compare Boeing and Lockheed.",
        observed_at=NOW,
    )
    event_route = omni.answer(
        sample,
        account_id=None,
        question="Why does this matter?",
        observed_at=NOW,
    ...
    general = omni.answer(
        sample, account_id=None, question="What is an RFQ?", observed_at=NOW
    )
    assert (
        "Defense researched account(s) with open quotes" in quotes.content
        and "SAMPLE commercial dataset" in quotes.content
    )
    assert (
        quotes.citations == ()
        and AssistantProvenance.DETERMINISTIC_DERIVATION in quotes.provenance
    )
    assert "quote/RFQ history" in history.content
    assert "Canonical comparison: Boeing and Lockheed Martin" in comparison.content
    assert comparison.context_used == {
        "account_id": "boeing",
...
def test_omni_conversation_event_precedence_continuity_and_staleness() -> None:
    ...
    first = omni.answer(
        ...
        question="Why does this matter?",
        observed_at=NOW,
        context={"surface": "INTELLIGENCE", "selected_event_id": first_event},
        intelligence_events=events,
    )
    continued = omni.answer(
        sample,
        account_id=None,
    ...
    superseded = omni.answer(
        ...
        context={
            "surface": "INTELLIGENCE",
            "selected_event_id": second_event,
            "conversation_referent": first.conversation_referent,
        },
        intelligence_events=events,
    )
    stale = omni.answer(
        ...
        observed_at=NOW,
        context={
            "conversation_referent": {"event_id": "missing-event", "route": "EVENT"}
        },
        intelligence_events=events,
    )
    ...
    assert (
        "Resolved organization: Boeing" in continued.content
        and continued.context_used["context_source"] == "conversation"
    )
    assert (
        superseded.context_used["event_id"] == second_event
        and superseded.context_used.get("context_source") is None
    )
...
def test_omni_conversation_facility_action_account_and_cleared_screen_context() -> None:
    ...
    owned = omni.answer(
        ...
        context={
            "surface": "ACCOUNTS",
            "conversation_referent": facility.conversation_referent,
        },
    )
    screen = omni.answer(
        sample,
        ...
        context={
            "surface": "ACCOUNTS",
            "visible_record_ids": ["lockheed-martin"],
            "conversation_referent": facility.conversation_referent,
        },
    )
    action = omni.answer(
    ...
    action_follow = omni.answer(
        sample,
        account_id=None,
        question="Which account is it for?",
        observed_at=NOW,
        context={"conversation_referent": action.conversation_referent},
        work_items=work.list(),
    )
    account = omni.answer(
        sample, account_id=None, question="Tell me about Boeing.", observed_at=NOW
    )
    account_follow = omni.answer(
        sample,
        account_id=None,
        question="Does it have open actions?",
        observed_at=NOW,
        context={"conversation_referent": account.conversation_referent},
        work_items=work.list(),
    )
    intelligence_follow = omni.answer(
        sample,
        account_id=None,
        question="Any Intelligence?",
        observed_at=NOW,
        context={"conversation_referent": account_follow.conversation_referent},
        work_items=work.list(),
    )
    assert (
        "Canonical parent account: Boeing" in owned.content
        and owned.context_used["context_source"] == "conversation"
    )
    assert (
        "Current Accounts view" in screen.content
        and "Boeing headquarters" not in screen.content
    )
    assert (
        "Canonical account: Boeing" in action_follow.content
...
def test_omni_conversation_comparison_relationship_and_ambiguity_are_bounded() -> None:
    ...
    score = omni.answer(
        sample,
        account_id=None,
        question="Which one has the higher score?",
        observed_at=NOW,
        context={"conversation_referent": comparison.conversation_referent},
        work_items=work.list(),
    )
    actions = omni.answer(
        sample,
        account_id=None,
        question="Which one has more open actions?",
        observed_at=NOW,
        context={"conversation_referent": comparison.conversation_referent},
        work_items=work.list(),
    )
    explicit = omni.answer(
        sample,
        account_id=None,
        question="Tell me about Northrop Grumman.",
        observed_at=NOW,
        context={"conversation_referent": comparison.conversation_referent},
    )
    ambiguous = omni.answer(
    ...
    other = omni.answer(
        ...
        context={
            "conversation_referent": {
                "account_id": "boeing",
                "comparison_account_ids": ["boeing", "lockheed-martin"],
                "route": "COMPARISON",
            }
        },
    )
    ...
    warmer = omni.answer(
        sample,
        account_id=None,
        question="Is there a warmer path?",
        observed_at=NOW,
        context={"conversation_referent": relationship.conversation_referent},
    )
    global_query = omni.answer(
        sample,
        account_id=None,
        question="Which Defense accounts have the highest scores?",
        observed_at=NOW,
        context={"conversation_referent": comparison.conversation_referent},
    )
    assert comparison.conversation_referent == {
        "comparison_account_ids": ["boeing", "lockheed-martin"],
        "route": "COMPARISON",
    }
    assert (
        "attractiveness-score dimension" in score.content
    ...
    assert (
        "Canonical relationship path" in warmer.content
